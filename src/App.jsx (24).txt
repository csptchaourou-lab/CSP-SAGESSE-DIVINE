...
function EspaceEnseignant({ eleves, setEleves, classeInitiale }) {
  ...
  const envoyerMessageParent = (eleveId, nomEleve) => {
    const texte = (msgParent[eleveId]||"").trim(); if(!texte) return;
    setEleves(prev=>prev.map(el=> el.id===eleveId ? {...el, messages:[...el.messages, {auteur: enseignantActif? enseignantActif.nom : `Enseignant ${maClasse}`, texte, date:formatDateFr()}]} : el));
    setMsgParent(prev=>({...prev, [eleveId]:""}));
    setConfirmation(`Message envoyé au parent de ${nomEleve}`);
  };
  const nav = [{ id: "classe", label: "Ma classe", icon: Users }, { id: "notes", label: "Notes & corrections", icon: ClipboardList }, { id: "absences", label: "Absences", icon: CalendarClock }, { id: "matieres", label: "Matières", icon: FileText }, { id: "messages_parents", label: "Messages → Parents", icon: MessageSquare }, { id: "annonces", label: "Annonces & messages", icon: Megaphone }];
  ...
  return (
    <Coquille role="enseignant" roleLabel={`Enseignant: ${enseignantActif? enseignantActif.nom : "Classe "+maClasse} - Classe ${maClasse}`} nav={nav} actif={onglet} setActif={setOnglet} onQuitter={() => {}}>
      ...
      {onglet === "messages_parents" && (<Panel title={`Messages → Parents - Classe ${maClasse}`} icon={MessageSquare}>
        <p className="mb-3 text-[13px]" style={{color:"#9A8B67"}}>NOUVEAU : Envoie un message directement au parent. Le parent le verra dans son espace Messages.</p>
        <div className="space-y-4">{mesEleves.map(e=>(
          <div key={e.id} className="rounded-xl border p-4" style={{borderColor:"#E7DEC8"}}>
            <div className="flex justify-between items-center mb-2">
              <div className="font-medium" style={{color:"#1B2A4A"}}>{e.prenom} {e.nom} - {e.id} - Parent: {e.parent?.nom}</div>
              <Badge>{e.messages.length} msgs</Badge>
            </div>
            <div className="mb-3 max-h-32 overflow-auto space-y-1">
              {e.messages.slice(-3).map((m,i)=><div key={i} className="text-[13px] rounded px-2 py-1" style={{background:"#F1ECDD"}}><b>{m.auteur}:</b> {m.texte} <span style={{color:"#9A8B67"}}>({m.date})</span></div>)}
            </div>
            <div className="flex gap-2">
              <input value={msgParent[e.id]||""} onChange={ev=>setMsgParent(prev=>({...prev, [e.id]: ev.target.value}))} placeholder={`Message pour parent de ${e.prenom}...`} className="flex-1 rounded-lg border px-3 py-2 text-[14px] outline-none" style={{borderColor:"#E7DEC8"}} />
              <Btn small variant="gold" icon={Send} onClick={()=>envoyerMessageParent(e.id, `${e.prenom} ${e.nom}`)}>Envoyer au parent</Btn>
            </div>
          </div>
        ))}</div>
      </Panel>)}
                  ...
                  <div className="flex flex-col gap-2 items-end">
                    ...
                    {d.statut==="en_attente" && <>
                      <Btn small variant="gold" onClick={()=>{
                        ...
                        setDemandes(prev=>prev.map(x=>x.id===d.id?{...x,statut:"validee", codeEleve:code}:x));
                        setEleves(prev=>[...prev,{
                    ...
                    {d.statut!=="en_attente" && <Badge tone={d.statut==="validee"?"vert":"ardoise"}>{d.statut} {d.codeEleve?`(${d.codeEleve})`:""}</Badge>}
                    </div>
                    <div className="flex gap-2">
                      {d.statut!=="en_attente" && <>
                        <Btn small variant="ghost" icon={Pencil} onClick={()=>setDemandes(prev=>prev.map(x=>x.id===d.id?{...x,statut:"en_attente"}:x))}>Modifier</Btn>
                        <Btn small variant="danger" icon={Trash2} onClick={()=>{
                          if(confirm(`Supprimer définitivement la demande de ${d.prenom} ${d.nom} ? ${d.statut==="validee"? "L'élève créé sera aussi supprimé." : ""}`)){
                            if(d.statut==="validee" && d.codeEleve){ setEleves(prev=>prev.filter(e=>e.id!==d.codeEleve)); }
                            setDemandes(prev=>prev.filter(x=>x.id!==d.id));
                          }
                        }}>Supprimer</Btn>
                      </>}
                      {d.statut==="en_attente" && <Btn small variant="danger" icon={Trash2} onClick={()=>{ if(confirm(`Supprimer demande ${d.prenom} ${d.nom}?`)) setDemandes(prev=>prev.filter(x=>x.id!==d.id)); }}>Supprimer</Btn>}
                    </div>
                  </div>
                </div>
              </div>
