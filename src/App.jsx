    ...
    setEleves((prev) => prev.map((e) => {
      ...
      return {
        ...
        frais: e.frais.map((f) => {
          ...
          return { ...f, paye: nouveau, mode, datePaiement: horodatageMsg(), payeLe: maintenant };
        }),
      };
    }));
    if (!resume.length) {
...
async function syncEleves(eleves, options = {}) {
  ...
  fraisLoc.forEach((f) => {
    ...
    if (!r) {
      ...
      String(r.date_paiement || "") !== String(f.date_paiement || f.paye_le || "")
    ) {
    ...
    try {
      await pgFetch(`/frais_eleve?id=eq.${f.id}`, {
        ...
        body: JSON.stringify({
          ...
          date_paiement: f.paye_le || f.date_paiement || f.datePaiement || null,
          libelle: f.libelle,
        }),
      });
    } catch (err) {
