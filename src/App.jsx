...
function genererBulletinPDF(enfant, ecole, moyenne) {
  ...
  win.document.write(`
    ...
    <br><p style="text-align:right">Fait à ${ecole.commune}, le ${formatDateHeureFr()}</p><p style="text-align:right">Le Directeur<br>${ecole.directeur}<br><br>___________________<br>Signature & Cachet</p>
...
function formatDateFr() {
  const d = new Date();
  const jour = d.toLocaleDateString("fr-FR", { timeZone: "Africa/Porto-Novo", day: "numeric" });
  const moisLong = d.toLocaleDateString("fr-FR", { timeZone: "Africa/Porto-Novo", month: "long" });
  const annee = d.toLocaleDateString("fr-FR", { timeZone: "Africa/Porto-Novo", year: "numeric" });
  return `${jour} ${moisLong.toUpperCase()} ${annee}`;
}
function formatHeureFr() {
  return new Date().toLocaleTimeString("fr-FR", { timeZone: "Africa/Porto-Novo", hour: "2-digit", minute: "2-digit" });
}
function formatDateHeureFr() {
  return `${formatDateFr()} à ${formatHeureFr()}`;
}
function formatDateEtHeureFr() {
  return formatDateHeureFr();
}
...
function EspaceDirecteur({ eleves=[], personnel=[], setPersonnel, annonces=[], setAnnonces, demandes=[], setDemandes, setEleves, isFondateur=false, ecole={}, setEcole, classes=["CI","CP","CE1","CE2","CM1","CM2"], setClasses, fraisTypes=[], setFraisTypes, anneeScolaire="", setAnneeScolaire, codes=[], setCodes, matieresParClasse={}, setMatieresParClasse, onQuitter }) {
  ...
  const supprimerUnEleve = async (eleveId, nomComplet) => {
    if (!confirm(`Supprimer definitivement ${nomComplet} ? Toutes ses notes, absences, frais, messages seront supprimes. Irreversible.`)) return;
    try {
      ...
      if (SUPABASE_CONFIGURE) {
        ...
        } catch (e) { console.warn("Erreur suppression Supabase:", e); }
      }
    } catch (err) { alert("Erreur: " + (err.message || String(err))); }
  };
...
function dedupePersonnel(liste) {
  const map = new Map();
  (liste || []).forEach((p) => {
    const cle = `${(p.nom||'').trim().toLowerCase()}||${(p.poste||'').trim().toLowerCase()}||${(p.classe||'').trim().toLowerCase()}`;
    map.set(cle, p);
  });
  return Array.from(map.values());
}nction dedupePersonnel(liste) {
