...
/** IDs d'élèves supprimés volontairement — persistés pour ne pas les réinjecter au refresh / polling. */
...
function ecrireElevesSupprimes(ids) {
  try {
    ...
    localStorage.setItem(CLE_ELEVES_SUPPRIMES, JSON.stringify(uniques));
  } catch (e) {
    console.warn("Impossible d'enregistrer les suppressions:", e);
  }
}
...
function marquerElevesSupprimes(idsAjout) {
  ...
  try {
    if (typeof window !== "undefined") {
      window.__cspElevesASupprimer = [...new Set([...(window.__cspElevesASupprimer || []).map(String), ...ids])];
    }
  } catch (e) {}
  return ids;
}

function retirerElevesSupprimesConfirmes(idsConfirmes) {
  if (!idsConfirmes || !idsConfirmes.length) return;
  const set = new Set(idsConfirmes.map(String));
  const restants = lireElevesSupprimes().filter((id) => !set.has(String(id)));
  ecrireElevesSupprimes(restants);
  try {
    if (typeof window !== "undefined") {
      window.__cspElevesASupprimer = (window.__cspElevesASupprimer || []).filter((id) => !set.has(String(id)));
    }
  } catch (e) {}
}

function filtrerElevesNonSupprimes(liste) {
  const bans = new Set(lireElevesSupprimes());
        ...
        if (d.eleves.length) {
          setEleves((prev) => {
            ...
            if ((!prev || prev.length === 0) && d.eleves.length) {
              ...
              return fusionnerEleves(prev, d.eleves);
            }
            return fusionnerEleves(prev, d.eleves);
          });
        }
