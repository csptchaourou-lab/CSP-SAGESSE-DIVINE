                  ...
                  <label className="inline-flex cursor-pointer items-center gap-1 rounded-lg px-2.5 py-1 text-[12px] font-semibold" style={{ background: "#0EA5E9", color: "#fff" }}>
                    ...
                    <input type="file" accept="image/*" className="hidden" onChange={(ev) => {
                      ...
                      compresserPhotoEleve(fichier).then((dataUrl) => {
                        setEleves((prev) => prev.map((x) => String(x.id) === String(e.id) ? { ...x, photoUrl: dataUrl } : x));
    ...
    <Coquille role="parent" roleLabel={`Parent de ${enfant.prenom} ${enfant.nom}`} nav={nav} actif={onglet} setActif={setOnglet}
      ...
      <Panel title="Profil de l'élève" icon={UserCircle2}>
        <div className="flex items-center gap-4">
          {enfant.photoUrl ? (
            <img src={enfant.photoUrl} alt="" className="h-20 w-20 rounded-full object-cover shadow-md" style={{ border: "3px solid #0EA5E9" }} />
          ) : (
    ...
    const appliquerDepuisBase = (d) => {
        ...
        // Si la base est vide (premier démarrage), on part des données démo une seule fois
        {
          ...
          if (d.eleves.length && cacheLocal.length) {
            setEleves(filtrerElevesNonSupprimes(fusionnerEleves(cacheLocal, d.eleves)));
          } else if (d.eleves.length) {
            setEleves(filtrerElevesNonSupprimes(d.eleves));
          } else if (cacheLocal.length) {
            // Base vide mais téléphone a des données → restaurer et renvoyer vers Supabase
            setEleves(filtrerElevesNonSupprimes(cacheLocal));
          } else {
            setEleves(SUPABASE_CONFIGURE ? [] : ELEVES_INIT);
          }
        }
        ...
        if (d.eleves.length) {
          setEleves((prev) => {
            ...
            if ((!prev || prev.length === 0) && d.eleves.length) {
              ...
              return filtrerElevesNonSupprimes(fusionnerEleves(prev, d.eleves));
            }
            return filtrerElevesNonSupprimes(fusionnerEleves(prev, d.eleves));
          });
        }
