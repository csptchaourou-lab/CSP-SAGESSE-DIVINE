...
function EspaceSecretaire({ eleves, setEleves, demandes, setDemandes, annonces, setAnnonces, onQuitter }) {
  ...
  const enregistrerPaiement = (eleve) => {
      ...
      return {
        ...
        frais: (e.frais || []).map((f) => {
          ...
          if (modeSaisie === "corriger") {
            // Le montant saisi = nouveau total payé (ex. ramener 20000 → 15000)
            nouveau = Math.max(0, val);
          } else {
            // Ajouter (positif) ou baisser (négatif)
            nouveau = Math.max(0, ancien + val);
          }
          if (nouveau === ancien) return f;
          resume.push(`${f.libelle}: ${ancien} → ${nouveau} F`);
          const maintenant = new Date().toISOString();
          // datePaiement = ISO (tri fiable en base) ; payeLe local pour la fusion
    ...
    setConfirmationPaiement((modeSaisie === "corriger" ? "Correction enregistrée — " : "Paiement enregistré — ") + resume.join(" · ") + " — Visible Parent + Directeur");
