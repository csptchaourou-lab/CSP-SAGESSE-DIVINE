...
function EspaceDirecteur({ eleves=[], personnel=[], setPersonnel, annonces=[], setAnnonces, demandes=[], setDemandes, setEleves, isFondateur=false, ecole={}, setEcole, classes=["CI","CP","CE1","CE2","CM1","CM2"], setClasses, fraisTypes=[], setFraisTypes, anneeScolaire="", setAnneeScolaire, codes={}, setCodes, matieresParClasse={}, setMatieresParClasse, onQuitter }) {
  ...
  const newPrenom = prompt("Prenom de l'eleve :", demande.prenom || "");
  if (newPrenom === null) return;
  const newNom = prompt("Nom de l'eleve :", demande.nom || "");
  if (newNom === null) return;
  const newSexe = prompt("Sexe (M / F) :", demande.sexe || "");
  if (newSexe === null) return;
  const newNaissance = prompt("Date de naissance :", demande.naissance || "");
  if (newNaissance === null) return;
  const newClasse = prompt("Classe souhaitee :", demande.classeSouhaitee || "");
  if (newClasse === null) return;
  const newParrain = prompt("Nom du parent / parrain :", demande.parrainNom || "");
  if (newParrain === null) return;
  const newTel = prompt("Telephone parent / parrain :", demande.parrainTelephone || "");
  if (newTel === null) return;
  const newMsg = prompt("Message / remarque :", demande.message || "");
  if (newMsg === null) return;
  setDemandes((prev) => prev.map((d) => d.id === demande.id ? {
    ...d,
    prenom: newPrenom.trim() || d.prenom,
    nom: newNom.trim() || d.nom,
    sexe: (newSexe || "").trim() || d.sexe,
    naissance: (newNaissance || "").trim() || d.naissance,
    classeSouhaitee: newClasse.trim() || d.classeSouhaitee,
    parrainNom: newParrain.trim() || d.parrainNom,
    parrainTelephone: newTel.trim() || d.parrainTelephone,
    message: newMsg.trim(),
  } : d));
  if (demande.codeEleve) {
    setEleves((prev) => prev.map((e) => String(e.id) === String(demande.codeEleve) ? {
      ...e,
      prenom: newPrenom.trim() || e.prenom,
      nom: newNom.trim() || e.nom,
      sexe: (newSexe || "").trim() || e.sexe,
      naissance: (newNaissance || "").trim() || e.naissance,
      classe: newClasse.trim() || e.classe,
      parent: {
        nom: newParrain.trim() || (e.parent && e.parent.nom) || "",
        telephone: newTel.trim() || (e.parent && e.parent.telephone) || "",
      },
    } : e));
  }
};
