  ...
  return (
    <div className="min-h-screen w-full" style={{ background: "linear-gradient(165deg, #ECFDF5 0%, #D1FAE5 35%, #FFFBEB 100%)" }}>
      <header className="sticky top-0 z-10 border-b backdrop-blur-md shadow-sm"
              style={{ borderColor: "#C7D2E5", background: "linear-gradient(90deg, #14532D 0%, #16A34A 45%, #B45309 100%)" }}>
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-2 px-4 py-3 sm:px-6">
          <div className="flex min-w-0 items-center gap-2.5">
            ...
            <div className="min-w-0">
              <div className="truncate font-serif text-[15px] font-semibold leading-none" style={{ color: "#F8FAFC" }}>{ecole.sigle}</div>
              <div className="text-[12px]" style={{ color: "#FDE68A" }}>{ecole.commune}</div>
            </div>
          </div>
...
function EspaceEnseignant({ eleves, setEleves, annonces=[], classeAssignee, onQuitter }) {
  ...
  const mesElevesTous = eleves
    ...
    .map((e) => ({ ...e, absences: filtrerAbsencesEleve(e.id, e.absences || []), notes: e.notes || [], messages: e.messages || [], frais: e.frais || [] }));
  ...
  const enregistrerNotesMatieres = (eleveId, nomEleve) => {
    ...
    setEleves((prev) => {
      const next = prev.map((el) => el.id === eleveId
...
function cleNoteObj(n) {
  return `${String(n.matiere || "").trim()}||${String(n.evaluation || "").trim()}||${String(n.mois || "").trim()}||${n.note20}`;
}
...
async function syncEleves(eleves, options = {}) {
  ...
  const cleNote = (n) => `${n.eleve_id || n.eleveId}||${String(n.matiere||"").trim()}||${String(n.evaluation||"").trim()}||${String(n.mois||"").trim()}||${n.note20}`;
  ...
  const fraisLoc = liste.flatMap((e) => (e.frais || []).map((f) => ({
    ...
    mode: f.mode ?? null,
    // Uniquement des colonnes existantes en base (pas de paye_le)
