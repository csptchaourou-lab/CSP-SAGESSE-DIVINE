...
function genererBulletinPDF(enfant, ecole, moyenne) {
  ...
  win.document.write(`
    ...
    <div class="header"><h1>${echapperHtml(ecole.sigle || "")} - ${echapperHtml(ecole.nom || "")}</h1><p>${echapperHtml(ecole.quartier || "")} - ${echapperHtml(ecole.commune || "")}, ${echapperHtml(ecole.departement || "")}</p><h2>Bulletin Scolaire${echapperHtml(ecole.devise ? " - " + ecole.devise : ""}</h2><p>Élève: <b>${echapperHtml(enfant.prenom)} ${echapperHtml(enfant.nom)}</b> | Classe: ${echapperHtml(enfant.classe)} | Code: ${echapperHtml(enfant.id)} | Naissance: ${echapperHtml(enfant.naissance || "—")}</p>${enfant.decision ? `<p style="margin-top:8px;padding:8px;background:#F8F4EA;border:1px solid #C9A227"><b>Décision de fin d'année :</b> ${echapperHtml(enfant.decision)}</p>` : ""}</div>
