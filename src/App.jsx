...
function imprimerListeClasse(elevesClasse, classe, ecole) {
  ...
  const cards = tries.map((e, i) =>
    ...
    "<div style=\"font-size:22px;font-weight:700;margin-top:4px\">" + (e.prenom || "—") + "</div>" +
    "<div style=\"font-size:17px;font-weight:600;color:#333\">" + (e.nom || "") + "</div>" +
            ...
            return (
              <div className="space-y-4">
                ...
                {elevesClasse.length === 0 && (
                  <p className="text-center text-[16px] py-6" style={{ color: "#9A8B67" }}>Aucun élève dans cette classe</p>
                )}
                <div className="space-y-3">
                    ...
                    <div key={e.id} className="rounded-2xl border-2 px-4 py-4" style={{ borderColor: "#E7DEC8", background: "#FFFFFF" }}>
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="font-serif text-[22px] font-bold leading-tight" style={{ color: "#1B2A4A" }}>{e.prenom || "—"}</div>
                          <div className="mt-0.5 text-[17px] font-semibold" style={{ color: "#3E3625" }}>{e.nom || ""}</div>
...
function EspaceSecretaire({ eleves, setEleves, demandes, setDemandes, annonces, setAnnonces, onQuitter }) {
  ...
  const statsFrais = useMemo(() => {
    const dus = eleves.reduce((s, e) => s + (e.frais || []).reduce((a, f) => a + (Number(f.du) || 0), 0), 0);
    const payes = eleves.reduce((s, e) => s + (e.frais || []).reduce((a, f) => a + (Number(f.paye) || 0), 0), 0);
    const reste = eleves.reduce((s, e) => s + (e.frais || []).reduce((a, f) => a + Math.max(0, (Number(f.du) || 0) - (Number(f.paye) || 0)), 0), 0);
    return { dus, payes, reste, nbEleves: eleves.length };
  }, [eleves]);
  ...
  return (
    <Coquille role="secretaire" roleLabel="Secrétaire" nav={nav} actif={onglet} setActif={setOnglet}
              onQuitter={onQuitter || (() => {})}>
      {/* Bilan financier global — visible sur tous les onglets secrétaire */}
      <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Stat label="Élèves inscrits" value={statsFrais.nbEleves} tone="ardoise" />
        <Stat label="Frais dus" value={`${statsFrais.dus.toLocaleString("fr-FR")} F`} tone="or" />
        <Stat label="Frais payés" value={`${statsFrais.payes.toLocaleString("fr-FR")} F`} tone="vert" />
        <Stat label="Frais restants" value={`${statsFrais.reste.toLocaleString("fr-FR")} F`} tone="or" />
      </div>
      ...
      {onglet === "annuaire" && (
        <Panel title="Annuaire — listes de classe" icon={Users}>
          ...
          {(() => {
            const elevesClasse = trierElevesAlpha(eleves.filter((e) => e.classe === classeAnnuaire));
            return (
              <div className="space-y-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  {elevesClasse.map((e, i) => (
                    <div key={e.id} className="rounded-2xl border-2 px-4 py-4" style={{ borderColor: "#99F6E4", background: "#F0FDFA" }}>
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="font-serif text-[22px] font-bold leading-tight" style={{ color: "#134E4A" }}>{e.prenom || "—"}</div>
                          <div className="mt-0.5 text-[17px] font-semibold" style={{ color: "#115E59" }}>{e.nom || ""}</div>
                          <div className="mt-2 flex flex-wrap items-center gap-2 text-[15px]">
