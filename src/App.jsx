...
async function pgFetch(chemin, options = {}) {
  try {
    ...
    clearTimeout(timer);
    if (!reponse.ok) {
      console.warn("Supabase:", reponse.status, chemin);
      return null;
    }
    ...
    console.warn("Supabase offline:", chemin, e.message);
    return null;
  }
}
const pgDeleteToutes = (table, col) => pgFetch(`/${table}?${col}=not.is.null`, { method: "DELETE", headers: { Prefer: "return=minimal" } });
...
const pgDeleteToutes_OLD = (table, colonneCle) => pgFetch(`/${table}?${colonneCle}=not.is.null`, { method: "DELETE", headers: { Prefer: "return=minimal" } });
...
async function chargerToutesLesDonnees() {
  ...
  const codes = { directeur: "1234", enseignant: "5678", secretaire: "9012" }; (codesRows || []).forEach((c) => { codes[c.role] = c.code; });
    const codesClasses = {};
  (codesRows||[]).forEach(r=>{ codesClasses[r.classe]=r.code; });
...
export default function EcoleConnecteeCSP() {
  ...
  useEffect(() => { let annule = false; const secu = setTimeout(()=>{ if(!annule){ setChargement(false); pretPourSync.current=true; }}, 3000); chargerToutesLesDonnees().then((d) => { if (annule || !d) return; setEcole(d.ecole||ECOLE_INIT); setAnneeScolaire(d.anneeScolaire||ANNEE_INIT); setCodes(d.codes||CODES_INIT); setCodesClasses(d.codesClasses||CODES_CLASSES_INIT); setClasses((d.classes&&d.classes.length)?d.classes:CLASSES_INIT); setMatieresParClasse(d.matieresParClasse||MATIERES_PAR_CLASSE_INIT); setPersonnel(d.personnel&&d.personnel.length?d.personnel:PERSONNEL_INIT); setFraisTypes(d.fraisTypes&&d.fraisTypes.length?d.fraisTypes:FRAIS_TYPES_INIT); setEleves(d.eleves||ELEVES_INIT); setAnnonces(d.annonces||ANNONCES_INIT); setMessagesInternes(d.messagesInternes||[]); setMessagesParents(d.messagesParents||[]); setDemandes(d.demandes||DEMANDES_INIT); setAnneesArchivees(d.anneesArchivees||[]); }).catch((err)=>{ if(!annule) setErreurChargement(err.message); }).finally(()=>{ if(annule) return; clearTimeout(secu); setChargement(false); setTimeout(()=>{pretPourSync.current=true;},0); }); return ()=>{ annule=true; clearTimeout(secu); }; }, []);
  ...
  const entrer = (roleChoisi, extra) => { let finale = extra; if(extra==="1002"||extra===1002) finale="Maternelle 2"; if(extra==="1001") finale="Maternelle 1"; if(extra==="1003") finale="CI"; if(extra==="1004") finale="CP"; if(extra==="1005") finale="CE1"; if(extra==="1006") finale="CE2"; if(extra==="1111") finale="CM1"; if(extra==="2222") finale="CM2"; if(extra==="3333") finale="Kponjè"; setRole(roleChoisi); if (roleChoisi === "parent") setEleveIdActif(extra); if (roleChoisi === "enseignant" && finale && typeof finale === "string" && classes.includes(finale)) { setClasseEnseignantActive(finale); } else if (roleChoisi === "enseignant" && extra && typeof extra === "string" && classes.includes(extra)) { setClasseEnseignantActive(extra); } else if (roleChoisi === "enseignant") { setClasseEnseignantActive(null); } };
