/* 
  CSP SAGESSE DIVINE - FORME EXACTE CONSERVÉE
  Corrections appliquées et vérifiées:
  1. Fondateur = pouvoir suprême: voit/modifie TOUS les codes (fondateur/directeur/secrétaire/enseignants), peut supprimer tous les élèves/personnel/demandes - Zone Fondateur existante ligne ~isFondateur
  2. Directeur = ne voit PAS code fondateur - déjà codé: isFondateur ? Champ : div "🔒 Code Fondateur caché"
  3. Enseignant = matière ajoutée via setMatieresParClasse -> même state global EcoleContext -> visible instantanément chez directeur/fondateur
  4. Secrétaire = inscription comme en ligne (origine secretariat, statut en_attente), PAS de bouton valider/modifier, PAS de code visible, encaissement via setEleves + setMessagesInternes -> sync auto Parent/Directeur/Fondateur, messages lecture seule
  5. Parent = bulletin PDF déjà présent genererBulletinPDF()
  6. Emploi du temps déjà présent dans toutes les vues
*/
...
const SUPABASE_URL = import.meta.env.VITE_SUPABASE_URL || "https://mwuquiyudypkxdxksihc.supabase.co";
const SUPABASE_ANON_KEY = import.meta.env.VITE_SUPABASE_ANON_KEY || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im13dXF1aXl1ZHlwa3hkeGtzaWhjIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODc2ODM5NzIsImV4cCI6MjEwMzI1OTk3Mn0.X6u7k6nqOMvHEFNNykli6lDJdhZxO0eZw8owxaFhgz4";
...
function EspaceEnseignant({ eleves, setEleves, annonces=[] }) {
  const { matieresParClasse, setMatieresParClasse, classes, setMessagesInternes, emploiDuTemps, setEmploiDuTemps } = useEcole();
...
export default function EcoleConnecteeCSP() {
  ...
  useEffect(() => {
    if (!SUPABASE_CONFIGURE) {
      // Mode local immédiat - pas de Supabase, plateforme utilisable sans clés
      setChargement(false);
    ...
    chargerToutesLesDonnees()
      ...
      .catch((err) => { 
        if (!annule) {
          // Si erreur Supabase, on continue en mode local
      ...
      .finally(() => {
        if (annule) return;
        setChargement(false);
        setTimeout(() => { pretPourSync.current = SUPABASE_CONFIGURE; }, 500);
      });
    return () => { annule = true; };
  }, []);
  ...
  const avecSuivi = (promesse) => {
    ...
    promesse.catch((err) => {
      ...
      if(m.includes('non configuré') || m.includes('fetch') || m.includes('network') || m.includes('404') || m.includes('not_found') || !navigator.onLine){ 
        console.warn('Sync offline', err); return; 
      }
      setErreurSync(err.message || String(err));
    });
  };

  useEffect(() => { if (pretPourSync.current && SUPABASE_CONFIGURE) avecSuivi(syncEcole(ecole, anneeScolaire)); }, [ecole, anneeScolaire]);
  useEffect(() => { if (pretPourSync.current && SUPABASE_CONFIGURE) avecSuivi(syncCodes(codes)); }, [codes]);
  useEffect(() => { if (pretPourSync.current && SUPABASE_CONFIGURE) avecSuivi(syncClasses(classes)); }, [classes]);
  useEffect(() => { if (pretPourSync.current && SUPABASE_CONFIGURE) avecSuivi(syncMatieresParClasse(matieresParClasse)); }, [matieresParClasse]);
  useEffect(() => { if (pretPourSync.current && SUPABASE_CONFIGURE) avecSuivi(syncPersonnel(personnel)); }, [personnel]);
  useEffect(() => { if (pretPourSync.current && SUPABASE_CONFIGURE) avecSuivi(syncFraisTypes(fraisTypes)); }, [fraisTypes]);
  useEffect(() => { if (pretPourSync.current && SUPABASE_CONFIGURE) avecSuivi(syncEleves(eleves)); }, [eleves]);
  useEffect(() => { if (pretPourSync.current && SUPABASE_CONFIGURE) avecSuivi(syncAnnonces(annonces)); }, [annonces]);
  useEffect(() => { if (pretPourSync.current && SUPABASE_CONFIGURE) avecSuivi(syncMessagesInternes(messagesInternes)); }, [messagesInternes]);
  useEffect(() => { if (pretPourSync.current && SUPABASE_CONFIGURE) avecSuivi(syncMessagesParents(messagesParents)); }, [messagesParents]);
  useEffect(() => { if (pretPourSync.current && SUPABASE_CONFIGURE) avecSuivi(syncDemandes(demandes)); }, [demandes]);
  useEffect(() => { if (pretPourSync.current && SUPABASE_CONFIGURE) avecSuivi(syncAnneesArchivees(anneesArchivees)); }, [anneesArchivees]);
