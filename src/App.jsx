...
function genererBulletinPDF(enfant, ecole, moyenne) {
  ...
  win.document.write(`
    ...
    <br><p style="text-align:right">Fait à ${ecole.commune}, le ${new Date().toLocaleDateString("fr-FR", {timeZone: "Africa/Porto-Novo"})} à ${new Date().toLocaleTimeString("fr-FR", {timeZone: "Africa/Porto-Novo", hour: "2-digit", minute: "2-digit"})}</p><p style="text-align:right">Le Directeur<br>${ecole.directeur}<br><br>___________________<br>Signature & Cachet</p>
...
function EspaceDirecteur({ eleves=[], personnel=[], setPersonnel, annonces=[], setAnnonces, demandes=[], setDemandes, setEleves, isFondateur=false, ecole={}, setEcole, classes=["CI","CP","CE1","CE2","CM1","CM2"], setClasses, fraisTypes=[], setFraisTypes, anneeScolaire="", setAnneeScolaire, codes=[], setCodes, matieresParClasse={}, setMatieresParClasse, onQuitter }) {
  ...
  // --- SUPPRESSION DEFINITIVE D'UN ELEVE (Fondateur + Directeur) ---
  const supprimerUnEleve = async (eleveId, nomComplet) => {
    if (!confirm(`Supprimer définitivement ${nomComplet} et toutes ses données (notes, absences, frais, messages) ? Irréversible.`)) return;
    try {
      // Supprime en local d'abord pour réactivité
      setEleves((prev) => prev.filter((e) => e.id !== eleveId));
      // Supprime dans Supabase si configuré
      if (SUPABASE_CONFIGURE) {
        try {
          await pgDeleteIn("notes", "eleve_id", [eleveId]);
          await pgDeleteIn("absences", "eleve_id", [eleveId]);
          await pgDeleteIn("frais_eleve", "eleve_id", [eleveId]);
          await pgDeleteIn("messages_eleve", "eleve_id", [eleveId]);
          await pgDeleteIn("messages_parents", "eleve_id", [eleveId]);
          await pgFetch(`/eleves?id=eq.${encodeURIComponent(eleveId)}`, { method: "DELETE", headers: { Prefer: "return=minimal" } });
        } catch (e) {
  ...
  const valider = (demande) => {
    ...
    // Une demande validée devient un élève : on la retire de la bibliothèque
    // afin qu’elle ne réapparaisse pas à la prochaine connexion.
...
const supprimerDemande = (demande) => {
  ...
  const envoyerMessageInterne = () => {
    ...
    setMessagesInternes((prev) => [
      { destinataire: nouveauMessageInterne.destinataire, texte: nouveauMessageInterne.texte.trim(), date: formatDateHeureFr() },
      ...prev,
    ]);
  ...
  const repondreParent = (index, msg) => {
    ...
    setEleves((prev) => prev.map((e) => e.id === msg.eleveId
      ? { ...e, messages: [...e.messages, { auteur: "Administration", texte, date: formatDateHeureFr() }] }
      : e));
    setMessagesParents((prev) => prev.map((m, i) => i === index ? { ...m, lu: true } : m));
          ...
          <div className="space-y-2">
            {suiviParClasse.map((s) => (
              <div key={s.classe} className="rounded-lg border px-4 py-3" style={{ borderColor: "#E7DEC8" }}>
                <div className="flex items-center justify-between">{/* Bouton Suppression Fondateur/Directeur */}
              <button onClick={() => supprimerUnEleve(e.id, `${e.prenom} ${e.nom}`)} title="Supprimer définitivement" className="ml-auto flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-bold" style={{background:"#8A2E2E", color:"#fff"}}><Trash2 size={12}/> Supprimer</button>
                  <div>
        ...
        <Panel title="Publier une annonce" icon={Megaphone}>
          <div className="mb-4 flex gap-2">
            ...
            <Btn variant="primary" icon={Plus} onClick={() => {
              ...
              setAnnonces([{ titre: nouvelleAnnonce, texte: nouvelleAnnonce, date: formatDateHeureFr() }, ...annonces]);
              setNouvelleAnnonce("");
            }}>Publier</Btn>
          </div>
          <div className="space-y-2">
...
function EspaceEnseignant({ eleves, setEleves, annonces=[], classeAssignee, onQuitter }) {
  ...
  const envoyerAppreciation = (eleveId, nomEleve) => {
    ...
    if (!texte) return;
    setEleves((prev) => prev.map((el) => el.id === eleveId ? { ...el, messages: [...el.messages, { auteur: "Enseignant(e)", texte, date: formatDateHeureFr() }] } : el));
    setMessagesInternes((prev) => [{ destinataire: "directeur", texte: "Appreciation " + maClasse + " pour " + nomEleve + ": " + texte, date: formatDateHeureFr() }, ...prev]);
...
function EspaceSecretaire({ eleves, setEleves, demandes, setDemandes, annonces, setAnnonces, onQuitter }) {
  ...
  const enregistrerPaiement = (eleve) => {
    ...
    setMessagesInternes((prev) => [{ destinataire: "directeur", texte: "Paiement Secretaire: " + eleve.prenom + " " + eleve.nom + " (" + eleve.classe + ") Total " + total + "F (" + mode + ")", date: formatDateHeureFr() }, ...prev]);
          ...
          <div className="flex gap-2">
            ...
            <Btn variant="primary" icon={Send} onClick={() => {
              ...
              setEleves((prev) => prev.map((e) => e.id === enfant.id
                ? { ...e, messages: [...e.messages, { auteur: "Vous", texte: message, date: formatDateHeureFr() }] }
                : e));
              setMessagesParents((prev) => [
                { eleveId: enfant.id, nomEleve: `${enfant.prenom} ${enfant.nom}`, classe: enfant.classe, texte: message, date: formatDateHeureFr(), lu: false },
                ...prev,
              ]);
              setMessage("");
            }}>Envoyer</Btn>
...
async function chargerToutesLesDonnees() {
  ...
  const eleves = (elevesRows || []).map((e) => ({
    ...
    frais: [
      ...(fraisEleveRows || []).filter((f) => f.eleve_id === e.id).map((f) => ({
      libelle: f.libelle, du: Number(f.du), paye: Number(f.paye), mode: f.mode, datePaiement: f.date_paiement,
      })),
      ...fraisTypes
        .filter((type) => (!type.classe || type.classe === e.classe) && !(fraisEleveRows || []).some((f) => f.eleve_id === e.id && f.libelle === type.libelle))
...
function dedupePersonnel(liste) {
  const vu = new Map();
  (liste || []).forEach((p) => {
    const cle = `${(p.nom||'').trim().toLowerCase()}||${(p.poste||'').trim().toLowerCase()}||${(p.classe||'').trim().toLowerCase()}||${(p.telephone||'').trim()}`;
    if (!vu.has(cle)) vu.set(cle, p);
    else {
      // si même personne mais téléphone différent, on garde le plus récent (nouveau)
      vu.set(cle, p);
    }
  });
  // Deuxième passe : dédoublonne sans téléphone pour éviter les vrais doublons
  const vu2 = new Map();
  for (const p of vu.values()) {
    const cle2 = `${(p.nom||'').trim().toLowerCase()}||${(p.poste||'').trim().toLowerCase()}||${(p.classe||'').trim().toLowerCase()}`;
    vu2.set(cle2, p);
  }
  return Array.from(vu2.values());
}
...
async function syncEleves(eleves) {
  ...
  // Ne supprimer un élève de la base que s'il a clairement été retiré d'une liste non vide
  // (on ne supprime plus en masse les notes de TOUS les élèves d'un coup de façon dangereuse)
  ...
  const cleFrais = (f) => `${f.eleve_id || f.eleveId}||${f.libelle}||${f.du}||${f.paye}`;
  const cleMsg = (m) => `${m.eleve_id || m.eleveId}||${m.texte}||${m.date}`;
  ...
  const setF = new Set(fraisRemote.map(cleFrais));
  ...
  const fraisNew = fraisLoc.filter((f) => !setF.has(cleFrais(f)));
  ...
  if (fraisNew.length) await pgInsert("frais_eleve", fraisNew);
...
export default function EcoleConnecteeCSP() {
  ...
  useEffect(() => {
    ...
    chargerToutesLesDonnees()
      .then((d) => {
        ...
        setTimeout(() => { sourceRemoteRef.current = false; }, 800);
      })
      .catch((err) => { 
        if (!annule) {
  ...
  useEffect(() => {
    ...
    const actualiser = async () => {
      try {
        ...
        setClasses(d.classes.length ? d.classes : CLASSES_INIT);
        setFraisTypes(d.fraisTypes.length ? d.fraisTypes : FRAIS_TYPES_INIT);
        if (d.eleves.length) setEleves(d.eleves);
        setPersonnel(d.personnel.length ? d.personnel : PERSONNEL_INIT);
        setEmploiDuTemps(d.emploiDuTemps || {});
        setAnneesArchivees(d.anneesArchivees || []);
        setAnnonces(d.annonces || []);
        setMessagesInternes(d.messagesInternes || []);
        setMessagesParents(d.messagesParents || []);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => avecSuivi(syncEleves(eleves)), 600);
