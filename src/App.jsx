...
function EspaceDirecteur({ eleves, personnel, setPersonnel, annonces, setAnnonces, demandes, setDemandes, setEleves }) {
  ...
  const [onglet, setOnglet] = useState("vueClasse");
  ...
  const ajouterFrais = () => {
    ...
    setEleves((prev) => prev.map((e) => e.frais.find((f) => f.libelle === nouveau.libelle) ? e : { ...e, frais: [...e.frais, { libelle: nouveau.libelle, du: nouveau.montant, paye: 0, mode: null, datePaiement: null }] }));
  ...
  const nav = [
    ...
    { id: "classes", label: "Classes (Modifiable)", icon: School },
    { id: "matieres", label: "Matières (Modifiable)", icon: FileText },
    { id: "frais", label: "Frais scolaires (Modifiable)", icon: Wallet },
    { id: "annee", label: "Année scolaire", icon: CalendarClock },
    { id: "personnel", label: "Personnel (Modifiable)", icon: Users },
    { id: "messagePublic", label: "Message Public ★", icon: Megaphone },
    { id: "messageInterne", label: "Message Interne Personnel", icon: MessageSquare },
  ...
  return (
    <Coquille role="directeur" roleLabel="Espace Administratif" nav={nav} actif={onglet} setActif={setOnglet}
      ...
      {onglet === "vueClasse" && (
        <Panel title="Vue par Classe - Toutes les infos Parents (Modifiable par Directeur)" icon={School}>
          ...
          {(() => {
            ...
            return <div className="space-y-3">
              ...
              {elevesClasse.map((e)=><div key={e.id} className="border rounded p-2" style={{borderColor:"#E7DEC8"}}>
                <div className="font-bold">{e.prenom} {e.nom} - {e.id}</div>
                <div className="text-[13px]">Notes: {e.notes.length} | Absences: {e.absences.length} | Messages: {e.messages.length} | Payé: {e.frais.reduce((a,f)=>a+f.paye,0)}F</div>
              </div>)}
            </div>;
          })()}
        </Panel>
...
function EspaceEnseignant({ eleves, setEleves }) {
  ...
  const envoyerAppreciation = (eleveId, nomEleve) => {
    ...
    setEleves((prev) => prev.map((el) => el.id === eleveId
      ...
      : el));
    setMessagesInternes((prev) => [{ destinataire:"directeur", texte:`📝 Appréciation ${maClasse} pour ${nomEleve}: ${texte}`, date:"aujourd'hui", classe: maClasse, type:"appreciation" }, ...prev]);
    setAppreciationSaisie((prev) => ({ ...prev, [eleveId]: "" }));
    setConfirmation(`Appréciation envoyée à Parent (${maClasse}) + Directeur`);
  };
...
function EspaceSecretaire({ eleves, setEleves, demandes, setDemandes, annonces, setAnnonces }) {
  ...
  const enregistrerPaiement = (eleve) => {
    ...
    if (total <= 0) return;
    const detail = Object.entries(montants).filter(([,v])=>parseFloat(v)>0).map(([l,v])=>`${l}:${v}F`).join(", ");
    setEleves((prev) => prev.map((e) => e.id === eleve.id
      ? { ...e, frais: e.frais.map((f) => { const ajout = parseFloat(montants[f.libelle]) || 0; if (ajout <= 0) return f; return { ...f, paye: f.paye + ajout, mode, datePaiement: "aujourd'hui" }; }) } : e));
    setMessagesInternes((prev) => [{ destinataire:"directeur", texte:`💰 Paiement Secrétaire: ${eleve.prenom} ${eleve.nom} (${eleve.classe}) - ${detail} - Total ${total}F (${mode})`, date:"aujourd'hui", type:"paiement" }, ...prev]);
...
function EspaceParent({ eleves, setEleves, eleveId, personnel }) {
  const { setMessagesParents, setMessagesInternes, annonces, anneesArchivees } = useEcole(); // Parent = lecture seule + archives
  ...
  const nav = [
    ...
    { id: "annonces", label: "Annonces École (Lecture seule)", icon: Megaphone },
    { id: "archives", label: "Archives Années Passées", icon: CalendarClock },
        ...
        <Panel title="Messages avec l'administration" icon={MessageSquare}>
          ...
          <div className="flex gap-2">
            ...
            <Btn variant="primary" icon={Send} onClick={() => {
              ...
              setMessagesInternes((prev) => [{ destinataire:"directeur", texte:`💬 Parent ${enfant.prenom} ${enfant.nom} (${enfant.classe}): ${message}`, date:"aujourd'hui", origine:"parent", type:"message_parent" }, ...prev]);
              setMessage("");
              alert("Message envoyé au Directeur uniquement - Parent = lecture seule pour le reste");
            }}>Envoyer au Directeur (Seule action autorisée)</Btn>
          </div>
        </Panel>
      )}
    </Coquille>
