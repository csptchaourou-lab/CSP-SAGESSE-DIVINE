          ...
          {(() => {
            ...
            return (
              <div className="space-y-3">
                ...
                {elevesClasse.map((e)=>{
                  const isEditing = editionForm.id===e.id;
                  return (
                    <div key={e.id} className="border rounded p-3" style={{borderColor: isEditing ? "#C9A227" : "#E7DEC8", background: isEditing ? "#FFFDF6" : "white"}}>
                      {!isEditing ? (
                        <>
                          <div className="flex justify-between items-start">
                            <div>
                              <div className="font-bold">{e.prenom} {e.nom} - <span className="font-mono text-[13px]">{e.id}</span></div>
                              <div className="text-[13px]" style={{color:"#5C5240"}}>Classe: {e.classe} | Naiss: {e.naissance||"—"} | Sexe: {e.sexe||"—"} | Parent: {e.parent?.nom} ({e.parent?.telephone})</div>
                              <div className="text-[13px]">Notes: {e.notes.length} | Absences: {e.absences.length} | Paye: {e.frais.reduce((a,f)=>a+f.paye,0)}F</div>
                            </div>
                            <div className="flex gap-2">
                              <button onClick={()=>{setDemandeEnEdition(null); setEditionForm({ id:e.id, prenom:e.prenom, nom:e.nom, classe:e.classe, naissance:e.naissance||"", sexe:e.sexe||"M", parrainNom:e.parent?.nom||"", parrainTelephone:e.parent?.telephone||"" });}} className="px-2 py-1 rounded text-[12px]" style={{background:"#1B2A4A", color:"white"}}>✏️ Modifier</button>
                              <button onClick={()=>{if(confirm(`Supprimer ${e.prenom} ${e.nom} définitivement ?`)){ setEleves((prev)=>prev.filter((el)=>el.id!==e.id)); setDemandes((prev)=>prev.map((d)=> d.codeEleve===e.id ? {...d, statut:"supprimee"} : d));}}} className="px-2 py-1 rounded text-[12px]" style={{background:"#8A2E2E", color:"white"}}>🗑️</button>
                            </div>
                          </div>
                        </>
                      ) : (
                        <div className="space-y-2">
                          <div className="grid grid-cols-2 gap-2">
                            <input value={editionForm.prenom} onChange={(ev)=>setEditionForm({...editionForm, prenom:ev.target.value})} placeholder="Prénom" className="border rounded px-2 py-1 text-[13px]" />
                            <input value={editionForm.nom} onChange={(ev)=>setEditionForm({...editionForm, nom:ev.target.value})} placeholder="Nom" className="border rounded px-2 py-1 text-[13px]" />
                            <select value={editionForm.sexe} onChange={(ev)=>setEditionForm({...editionForm, sexe:ev.target.value})} className="border rounded px-2 py-1 text-[13px]"><option value="M">Masculin</option><option value="F">Féminin</option></select>
                            <input type="date" value={editionForm.naissance} onChange={(ev)=>setEditionForm({...editionForm, naissance:ev.target.value})} className="border rounded px-2 py-1 text-[13px]" />
                            <select value={editionForm.classe} onChange={(ev)=>setEditionForm({...editionForm, classe:ev.target.value})} className="border rounded px-2 py-1 text-[13px]">{classes.map((cl)=><option key={cl} value={cl}>{cl}</option>)}</select>
                            <input value={editionForm.parrainNom} onChange={(ev)=>setEditionForm({...editionForm, parrainNom:ev.target.value})} placeholder="Nom parent" className="border rounded px-2 py-1 text-[13px]" />
                            <input value={editionForm.parrainTelephone} onChange={(ev)=>setEditionForm({...editionForm, parrainTelephone:ev.target.value})} placeholder="Téléphone parent" className="border rounded px-2 py-1 text-[13px]" />
                          </div>
                          <div className="flex gap-2">
                            <button onClick={()=>{ setEleves((prev)=>prev.map((el)=> el.id===e.id ? {...el, prenom:editionForm.prenom.trim(), nom:editionForm.nom.trim(), classe:editionForm.classe, naissance:editionForm.naissance, sexe:editionForm.sexe, parent:{nom:editionForm.parrainNom, telephone:editionForm.parrainTelephone}} : el)); setDemandes((prev)=>prev.map((d)=> d.codeEleve===e.id ? {...d, prenom:editionForm.prenom.trim(), nom:editionForm.nom.trim(), classeSouhaitee:editionForm.classe, naissance:editionForm.naissance, sexe:editionForm.sexe, parrainNom:editionForm.parrainNom, parrainTelephone:editionForm.parrainTelephone} : d)); setEditionForm({});}} className="px-3 py-1.5 rounded text-[13px]" style={{background:"#C9A227", color:"#1B2A4A"}}>💾 Enregistrer</button>
                            <button onClick={()=>setEditionForm({})} className="px-3 py-1.5 rounded border text-[13px]" style={{borderColor:"#E7DEC8"}}>Annuler</button>
                          </div>
                        </div>
                      )}
                    </div>
...
function EspaceFondateur({ eleves, personnel, setPersonnel, annonces, setAnnonces, demandes, setDemandes, setEleves }) {
  ...
  const nav = [
    ...
    { id: "codes", label: "Tous les Codes", icon: KeyRound },
    { id: "eleves", label: "Tous les Eleves", icon: Users },
    { id: "personnel", label: "Personnel", icon: Users },
    ...
    { id: "parametres", label: "Parametres Ecole", icon: Settings },
  ];
  ...
  return (
    <Coquille role="fondateur" roleLabel="Espace Fondateur - Acces Total" nav={nav} actif={onglet} setActif={setOnglet} onQuitter={()=>{}}>
      {onglet==="dashboard" && (
        <>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <Stat label="Eleves" value={stats.totalEleves} tone="ardoise" />
            <Stat label="Personnel" value={stats.totalPersonnel} tone="ardoise" />
            <Stat label="Classes" value={stats.totalClasses} tone="or" />
            <Stat label="Demandes" value={stats.totalDemandes} tone="or" />
          </div>
          <Panel title="Pouvoirs Fondateur - ATTENTION" icon={BadgeCheck}>
            <p className="text-[15px] mb-3" style={{color:"#8A2E2E"}}>Tu es Fondateur : tu peux tout voir, tout modifier, tout supprimer. Tu peux changer le code Directeur, Secretaire, Fondateur, et les 8 codes enseignants. Tu as la main sur toute la plateforme.</p>
            <div className="grid grid-cols-2 gap-2">
              <button onClick={()=>{ if(!confirm("Vider TOUTE la base (eleves, personnel, annonces, messages) ? Codes gardes.")) return; setEleves([]); setPersonnel([]); setAnnonces([]); setDemandes([]); setMessagesInternes([]); setMessagesParents([]); alert("Base videe"); }} className="bg-red-600 text-white px-3 py-2 rounded">🗑️ Vider toute la base</button>
              <button onClick={()=>{ if(!confirm("Supprimer toutes les classes ?")) return; setClasses([]); setMatieresParClasse({}); }} className="bg-[#1B2A4A] text-white px-3 py-2 rounded">Supprimer toutes les classes</button>
            </div>
          </Panel>
        </>
      )}
      ...
      {onglet==="codes" && (
        <Panel title="Tous les Codes d'Acces - Modifiable par Fondateur" icon={KeyRound}>
          <div className="space-y-4">
            <div className="border rounded p-3" style={{borderColor:"#E7DEC8"}}>
              <h4 className="font-bold mb-2">Codes Principaux</h4>
              <div className="space-y-2">
                <div className="flex gap-2 items-center"><span className="w-32">Directeur</span><input value={codes.directeur} onChange={(e)=>setCodes({...codes, directeur:e.target.value})} className="border rounded px-2 py-1 flex-1" /><span className="text-[12px]" style={{color:"#9A8B67"}}>code actuel</span></div>
                <div className="flex gap-2 items-center"><span className="w-32">Secretaire</span><input value={codes.secretaire} onChange={(e)=>setCodes({...codes, secretaire:e.target.value})} className="border rounded px-2 py-1 flex-1" /></div>
                <div className="flex gap-2 items-center"><span className="w-32">Fondateur</span><input value={codes.fondateur} onChange={(e)=>setCodes({...codes, fondateur:e.target.value})} className="border rounded px-2 py-1 flex-1 bg-yellow-50" /><span className="text-[12px] text-red-600">TON CODE - ne le partage pas</span></div>
                <div className="flex gap-2 items-center"><span className="w-32">Enseignant generique</span><input value={codes.enseignant} onChange={(e)=>setCodes({...codes, enseignant:e.target.value})} className="border rounded px-2 py-1 flex-1" /></div>
              </div>
            </div>
            <div className="border rounded p-3" style={{borderColor:"#E7DEC8"}}>
              <h4 className="font-bold mb-2">8 Codes Enseignants Perso (1 par classe)</h4>
              {(codes.enseignants||[]).map((ens,idx)=>(
                <div key={idx} className="flex gap-2 items-center mb-2">
                  <input value={ens.nom} onChange={(e)=>{ const arr=[...codes.enseignants]; arr[idx].nom=e.target.value; setCodes({...codes, enseignants:arr}); }} className="border rounded px-2 py-1 w-32 text-[13px]" placeholder="Nom" />
                  <select value={ens.classe} onChange={(e)=>{ const arr=[...codes.enseignants]; arr[idx].classe=e.target.value; setCodes({...codes, enseignants:arr}); }} className="border rounded px-2 py-1 w-24 text-[13px]">{classes.map((cl)=><option key={cl} value={cl}>{cl}</option>)}</select>
                  <input value={ens.code} onChange={(e)=>{ const arr=[...codes.enseignants]; arr[idx].code=e.target.value; setCodes({...codes, enseignants:arr}); }} className="border rounded px-2 py-1 flex-1 font-mono text-[13px]" />
                  <button onClick={()=>{ setCodes({...codes, enseignants: codes.enseignants.filter((_,i)=>i!==idx)}); }} className="text-red-600"><Trash2 size={14}/></button>
                </div>
              ))}
              <button onClick={()=>setCodes({...codes, enseignants:[...codes.enseignants, {nom:"Nouvel Enseignant", classe:classes[0]||"CI", code:"ENS-NEW-"+Date.now().toString().slice(-3)}]})} className="bg-[#1B2A4A] text-white px-3 py-1 rounded text-[13px]"><Plus size={12}/> Ajouter Enseignant</button>
            </div>
          </div>
        </Panel>
      )}
      ...
      {onglet==="eleves" && (
        <Panel title="Tous les Eleves - Fondateur peut tout modifier/supprimer" icon={Users}>
          <div className="space-y-2">
            {eleves.map((e)=>(
              <div key={e.id} className="border rounded p-2 flex justify-between items-center" style={{borderColor:"#E7DEC8"}}>
                <div><b>{e.prenom} {e.nom}</b> - {e.classe} - {e.id} - {e.parent?.telephone}</div>
                <div className="flex gap-1">
                  <button onClick={()=>{ if(!confirm("Supprimer cet eleve ?")) return; setEleves(prev=>prev.filter((el)=>el.id!==e.id)); }} className="bg-red-600 text-white px-2 py-1 rounded text-[12px]">Supprimer</button>
                  <button onClick={()=>{ const newClasse=prompt("Nouvelle classe pour "+e.prenom, e.classe); if(newClasse) setEleves(prev=>prev.map((el)=>el.id===e.id?{...el, classe:newClasse}:el)); }} className="bg-[#1B2A4A] text-white px-2 py-1 rounded text-[12px]">Changer classe</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {onglet==="personnel" && (
        <Panel title="Personnel" icon={Users}>
          <div className="space-y-2">
            {personnel.map((p,i)=>(
              <div key={i} className="border rounded p-2 flex justify-between" style={{borderColor:"#E7DEC8"}}>
                <span>{p.nom} - {p.poste} - {p.classe} - {p.telephone}</span>
                <button onClick={()=>setPersonnel(prev=>prev.filter((_,idx)=>idx!==i))} className="text-red-600"><Trash2 size={14}/></button>
              </div>
            ))}
          </div>
        </Panel>
      ...
      {onglet==="classes" && (
        <Panel title="Classes & Matieres" icon={School}>
          <div className="flex gap-2 mb-3 flex-wrap">{classes.map((cl,idx)=><div key={cl} className="border rounded px-2 py-1 flex gap-1 items-center">{cl}<button onClick={()=>setClasses(prev=>prev.filter((_,i)=>i!==idx))} className="text-red-600 ml-1"><Trash2 size={10}/></button></div>)}</div>
          <div className="space-y-3">
            {Object.entries(matieresParClasse).map(([cl, mats])=>(
              <div key={cl} className="border rounded p-2" style={{borderColor:"#E7DEC8"}}><b>{cl}</b>: {mats.map((m)=>m.nom).join(", ")}</div>
            ))}
          </div>
        </Panel>
      )}
      ...
      {onglet==="frais" && (
        <Panel title="Frais Scolaires" icon={Wallet}>
          {fraisTypes.map((f,i)=><div key={i} className="border rounded p-2 mb-1 flex justify-between"><span>{f.libelle} - {f.montant}F</span><button onClick={()=>setFraisTypes(prev=>prev.filter((_,idx)=>idx!==i))} className="text-red-600"><Trash2 size={12}/></button></div>)}
        </Panel>
      )}

      {onglet==="annonces" && (
        <Panel title="Annonces Ecole - Fondateur peut supprimer" icon={Megaphone}>
          {annonces.map((a,i)=><div key={i} className="border rounded p-2 mb-1 flex justify-between"><div><b>{a.titre}</b><p className="text-[13px]">{a.texte}</p></div><button onClick={()=>setAnnonces(prev=>prev.filter((_,idx)=>idx!==i))} className="text-red-600"><Trash2 size={14}/></button></div>)}
        </Panel>
      )}

      {onglet==="demandes" && (
        <Panel title="Demandes Inscription" icon={Inbox}>
          {demandes.map((d,i)=><div key={i} className="border rounded p-2 mb-1 flex justify-between"><span>{d.prenom} {d.nom} - {d.classeSouhaitee} - {d.statut}</span><button onClick={()=>setDemandes(prev=>prev.filter((_,idx)=>idx!==i))} className="text-red-600"><Trash2 size={12}/></button></div>)}
        </Panel>
      )}

      {onglet==="messages" && (
        <Panel title="Messages Parents & Internes" icon={MessageSquare}>
          <h4 className="font-bold">Messages Parents</h4>
          {messagesParents.map((m,i)=><div key={i} className="border rounded p-2 mb-1">{m.nomEleve} - {m.classe} : {m.texte} <button onClick={()=>setMessagesParents(prev=>prev.filter((_,idx)=>idx!==i))} className="text-red-600 ml-2"><Trash2 size={10}/></button></div>)}
          <h4 className="font-bold mt-4">Messages Internes</h4>
          {messagesInternes.map((m,i)=><div key={i} className="border rounded p-2 mb-1">{m.destinataire}: {m.texte} <button onClick={()=>setMessagesInternes(prev=>prev.filter((_,idx)=>idx!==i))} className="text-red-600 ml-2"><Trash2 size={10}/></button></div>)}
        </Panel>
      )}

      {onglet==="parametres" && (
        <Panel title="Parametres Ecole - Fondateur" icon={Settings}>
          <div className="space-y-2">
            <input value={ecole.nom} onChange={(e)=>setEcole({...ecole, nom:e.target.value})} placeholder="Nom ecole" className="border rounded px-2 py-1 w-full" />
            <input value={ecole.sigle} onChange={(e)=>setEcole({...ecole, sigle:e.target.value})} placeholder="Sigle" className="border rounded px-2 py-1 w-full" />
            <input value={ecole.quartier} onChange={(e)=>setEcole({...ecole, quartier:e.target.value})} placeholder="Quartier" className="border rounded px-2 py-1 w-full" />
            <input value={ecole.telephone} onChange={(e)=>setEcole({...ecole, telephone:e.target.value})} placeholder="Telephone" className="border rounded px-2 py-1 w-full" />
            <input value={anneeScolaire} onChange={(e)=>setAnneeScolaire(e.target.value)} placeholder="Annee scolaire" className="border rounded px-2 py-1 w-full" />
          </div>
          <div className="mt-4 border-t pt-4">
            <h4 className="font-bold text-red-600">Zone Danger - Fondateur</h4>
            <button onClick={()=>{ if(!confirm("Vider TOUT ?")) return; setEleves([]); setPersonnel([]); setAnnonces([]); setDemandes([]); setMessagesInternes([]); setMessagesParents([]); setFraisTypes([]); setClasses([]); setMatieresParClasse({}); setAnneesArchivees([]); }} className="bg-red-700 text-white px-4 py-2 rounded mt-2">🔥 REMISE A ZERO TOTALE PLATEFORME</button>
          </div>
        </Panel>
      )}
    </Coquille>
