...
function imprimerListeClasse(elevesClasse, classe, ecole) {
  ...
  const cards = tries.map((e, i) =>
    ...
    "<div style=\"font-size:22px;font-weight:700;margin-top:4px\">" + (e.nom || "—") + "</div>" +
    "<div style=\"font-size:17px;color:#444\">" + (e.prenom || "") + "</div>" +
    "<div style=\"margin-top:8px;font-size:15px\">" +
    "<span style=\"background:#EEF2FF;padding:4px 10px;border-radius:6px;font-family:monospace;font-weight:700\">" + (e.id || "") + "</span> " +
    "<span style=\"background:#F1ECDD;padding:4px 10px;border-radius:6px;font-weight:600\">" + (e.classe || classe) + "</span>" +
    "</div></div>"
  ).join("");
  win.document.write("<!DOCTYPE html><html><head><title>Liste " + classe + "</title>" +
    "<style>body{font-family:Arial,sans-serif;padding:20px;color:#1B2A4A} h1{font-size:20px}</style>" +
    "</head><body>" +
    "<h1>" + ((ecole && ecole.sigle) || "École") + " — Liste de classe</h1>" +
    "<p style=\"font-size:16px\"><b>Classe :</b> " + classe + " &nbsp;|&nbsp; <b>Effectif :</b> " + tries.length + " &nbsp;|&nbsp; " + new Date().toLocaleDateString("fr-FR") + "</p>" +
...
function genererBulletinPDF(enfant, ecole, moyenne) {
  ...
  win.document.write(`
    ...
    <div class="header"><h1>${ecole.sigle || ""} - ${ecole.nom || ""}</h1><p>${ecole.quartier || ""} - ${ecole.commune || ""}, ${ecole.departement || ""}</p><h2>Bulletin Scolaire${ecole.devise ? " - " + ecole.devise : ""}</h2><p>Élève: <b>${enfant.prenom} ${enfant.nom}</b> | Classe: ${enfant.classe} | Code: ${enfant.id} | Naissance: ${enfant.naissance || "—"}</p>${enfant.decision ? `<p style="margin-top:8px;padding:8px;background:#F8F4EA;border:1px solid #C9A227"><b>Décision de fin d'année :</b> ${enfant.decision}</p>` : ""}</div>
...
function genererRecuPaiement({ ecole, eleve, lignes, mode, modeSaisie, totalMouvement }) {
  ...
  const html = "<!DOCTYPE html><html><head><meta charset='utf-8'/><title>" + numRecu + "</title>" +
    ...
    "<div class='box'><div><b>Élève :</b> " + (eleve.prenom || "") + " " + (eleve.nom || "") + "</div>" +
  ...
  try { journaliser("Reçu émis " + numRecu + " — " + (eleve.prenom || "") + " " + (eleve.nom || ""), "secretaire"); } catch (e) {}
}
...
function Badge({ children, tone = "or" }) {
  const tones = {
    ...
    ardoise: { bg: "linear-gradient(135deg, #FCE7F3, #F9A8D4)", fg: "#9D174D", ring: "#DB2777" },
  };
...
function Stat({ label, value, tone }) {
  const tones = { ardoise: "#9D174D", or: "#B45309", vert: "#14532D" };
  const bg = {
    ardoise: "linear-gradient(145deg, #FDF2F8 0%, #F9A8D4 100%)",
