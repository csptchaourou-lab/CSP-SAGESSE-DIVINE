...
const supprimerDemande = (demande) => {
  ...
  const repondreParent = (index, msg) => {
    ...
    setEleves((prev) => prev.map((e) => e.id === msg.eleveId
      ...
      : e));
    // Visible aussi côté directeur dans la liste des messages
    setMessagesParents((prev) => [
      { eleveId: msg.eleveId, nomEleve: msg.nomEleve, classe: msg.classe, texte: `Réponse admin : ${texte}`, date, lu: true, deAdmin: true },
      ...prev.map((m, i) => i === index ? { ...m, lu: true } : m),
    ]);
  ...
  const envoyerMessageAuParent = () => {
    ...
    setEleves((prev) => prev.map((e) => e.id === msgAdminEleveId
      ...
      : e));
    setMessagesParents((prev) => [
      { eleveId: eleve.id, nomEleve: `${eleve.prenom} ${eleve.nom}`, classe: eleve.classe, texte: `Message admin : ${texte}`, date, lu: true, deAdmin: true },
      ...prev,
    ]);
    setMsgAdminTexte("");
  };
      ...
      {onglet === "messagesParents" && (
        <Panel title="Messages parents" icon={MessageSquare}>
          ...
          <div className="mb-4">
            ...
            <div className="space-y-2">
              {eleves.flatMap((e) => (e.messages || []).filter((m) => m.auteur === "Administration" || m.auteur === "Enseignant(e)").map((m, i) => (
                <div key={`${e.id}-out-${i}`} className="rounded-lg border px-3 py-2 text-[14px]" style={{ borderColor: "#DDEDE3", background: "#F3FAF5" }}>
          ...
          <div className="space-y-3">
            ...
            {messagesParents.map((m, i) => (
              <div key={i} className="rounded-lg border px-4 py-3" style={{ borderColor: "#E7DEC8", background: m.deAdmin ? "#F3FAF5" : "transparent" }}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-[15px]" style={{ color: "#1B2A4A" }}>{m.nomEleve}</span>
                    <Badge tone="ardoise">{m.classe}</Badge>
                    {m.deAdmin ? <Badge tone="vert">Envoyé</Badge> : (!m.lu && <Badge tone="or">Non lu</Badge>)}
                  </div>
                  <span className="text-[13px]" style={{ color: "#9A8B67" }}>{m.date}</span>
                </div>
...
function cleMsgObj(m) {
  return `${m.auteur || ""}||${m.texte || ""}||${m.date || ""}`;
}
  ...
  useEffect(() => {
    ...
    const actualiser = async () => {
      try {
        ...
        setDemandes((prev) => (d.demandes && d.demandes.length ? d.demandes : prev));
