  ...
  const supprimerEleve = (eleve) => {
    ...
    try {
      if (typeof window !== "undefined") {
        window.__cspElevesASupprimer = [...(window.__cspElevesASupprimer || []), eleve.id];
      }
    } catch (e) {}
...
async function syncEleves(eleves, options = {}) {
  ...
  // Sinon un appareil avec 1 élève effaçait les 299 autres en base.
  const aSupprimerExplicit = (options.supprimerIds || []).filter(Boolean);
...
export default function EcoleConnecteeCSP() {
      ...
      .then((d) => {
        ...
        // Si la base est vide (premier démarrage), on part des données démo une seule fois
        {
          ...
          if (d.eleves.length && cacheLocal.length) {
            setEleves(fusionnerEleves(cacheLocal, d.eleves));
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      let supprimerIds = [];
      try {
        supprimerIds = (typeof window !== "undefined" && window.__cspElevesASupprimer) ? [...window.__cspElevesASupprimer] : [];
        if (typeof window !== "undefined") window.__cspElevesASupprimer = [];
      } catch (e) {}
      avecSuivi(syncEleves(elevesRef.current || [], { vider, supprimerIds }));
