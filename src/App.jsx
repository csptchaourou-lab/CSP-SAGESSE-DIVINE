...
async function chargerToutesLesDonnees() {
  ...
  const eleves = (elevesRows || []).map((e) => ({
    ...
    frais: (() => {
      const depuisBase = (fraisEleveRows || []).filter((f) => f.eleve_id === e.id).map((f) => ({
      ...
      // Une seule ligne par libellé (évite doublons secrétaire / parent)
      return Array.from(new Map([...depuisBase, ...depuisCatalogue].map((f) => [
        String(f.libelle || "").trim().toLowerCase(), f
      ])).values());
    })(),
