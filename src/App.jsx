...
function imprimerListeClasse(elevesClasse, classe, ecole) {
  ...
  const rows = tries.map((e, i) =>
    "<tr><td>" + (i + 1) + "</td><td>" + (e.nom || "") + " " + (e.prenom || "") + "</td><td>" + (e.id || "") + "</td><td>" + (e.classe || classe) + "</td><td>" + (e.sexe || "") + "</td><td>" + (e.naissance || "") + "</td></tr>"
  ).join("");
  win.document.write("<!DOCTYPE html><html><head><title>Liste " + classe + "</title>" +
    "<style>body{font-family:Arial;padding:20px;color:#1B2A4A} table{width:100%;border-collapse:collapse;margin-top:12px} th,td{border:1px solid #ccc;padding:8px;text-align:left} th{background:#1B2A4A;color:#fff} h1{font-size:18px}</style>" +
    "</head><body>" +
    "<h1>" + ((ecole && ecole.sigle) || "École") + " — Liste de classe (ordre alphabétique)</h1>" +
    "<p><b>Classe :</b> " + classe + " &nbsp;|&nbsp; <b>Effectif :</b> " + tries.length + " &nbsp;|&nbsp; " + new Date().toLocaleDateString("fr-FR") + "</p>" +
    "<table><thead><tr><th>N°</th><th>Nom et prénom</th><th>Code</th><th>Classe</th><th>Sexe</th><th>Naissance</th></tr></thead>" +
    "<tbody>" + (rows || "<tr><td colspan=6>Aucun élève</td></tr>") + "</tbody></table>" +
      ...
      {onglet === "dashboard" && (
        <>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            ...
            <Stat label="Frais payés" value={`${stats.payes.toLocaleString("fr-FR")} F`} tone="vert" />
            ...
            return (
              <div className="space-y-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="font-serif text-lg" style={{ color: "#1B2A4A" }}>Classe {classeVue} — {elevesClasse.length} élève(s)</div>
                  <div className="flex flex-wrap gap-2">
                    <Btn variant="gold" small icon={ClipboardList} onClick={() => {
                      const t = texteListeClasse(elevesClasse, classeVue);
                      if (navigator.clipboard && navigator.clipboard.writeText) {
                    ...
                    <Btn variant="ghost" small icon={FileText} onClick={() => imprimerListeClasse(elevesClasse, classeVue, ecole)}>Imprimer</Btn>
                  </div>
                </div>
                <div className="overflow-x-auto rounded-xl border" style={{ borderColor: "#E7DEC8" }}>
                  <table className="w-full text-left text-[14px]">
                    ...
                    <tbody>
                      {elevesClasse.length === 0 && (
                        <tr><td colSpan={5} className="px-3 py-4 text-center" style={{ color: "#9A8B67" }}>Aucun élève dans cette classe</td></tr>
                      )}
                      {elevesClasse.map((e, i) => (
                        <tr key={e.id} style={{ borderTop: "1px solid #E7DEC8" }}>
                          <td className="px-3 py-2">{i + 1}</td>
                          <td className="px-3 py-2 font-semibold">{e.prenom} {e.nom}</td>
                          <td className="px-3 py-2 font-mono text-[12px]">{e.id}</td>
                          <td className="px-3 py-2">{e.sexe || "—"}</td>
                          <td className="px-3 py-2">{e.naissance || "—"}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        <Panel title="Annuaire — listes de classe" icon={Users}>
          ...
          {(() => {
            ...
            return (
              <div className="space-y-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="font-serif text-lg font-bold" style={{ color: "#134E4A" }}>
                    Classe {classeAnnuaire} — {elevesClasse.length} élève(s) — ordre alphabétique
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <Btn variant="gold" small icon={ClipboardList} onClick={() => {
                    ...
                    <Btn variant="ghost" small icon={FileText} onClick={() => imprimerListeClasse(elevesClasse, classeAnnuaire, ecole)}>Imprimer</Btn>
                  </div>
                </div>
                <div className="overflow-x-auto rounded-xl border" style={{ borderColor: "#99F6E4" }}>
                  <table className="w-full text-left text-[14px]">
                    <thead style={{ background: "#0F766E", color: "#fff" }}>
                      <tr>
                        ...
                        <th className="px-3 py-2">Code élève</th>
                    ...
                    <tbody>
                      ...
                      {elevesClasse.map((e, i) => (
                        <tr key={e.id} style={{ borderTop: "1px solid #CCFBF1" }}>
                          <td className="px-3 py-2">{i + 1}</td>
                          <td className="px-3 py-2 font-semibold">{e.nom}</td>
                          <td className="px-3 py-2">{e.prenom}</td>
                          <td className="px-3 py-2 font-mono text-[13px]" style={{ color: "#0F766E" }}>{e.id}</td>
                          <td className="px-3 py-2">{e.classe}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
