...
// ===== NETTOYAGE DOUBLONS SUPABASE - NOUVEAU =====
function dedupFraisParLibelle(fraisArray) {
  const map = new Map();
  (fraisArray||[]).forEach(f => {
    const key = (f.libelle||'').trim();
    if (!key) return;
    if (!map.has(key)) map.set(key, { ...f });
    else {
      const prev = map.get(key);
      map.set(key, { ...prev, du: Math.max(prev.du||0, f.du||0), paye: (prev.paye||0)+(f.paye||0) });
    }
  });
  return Array.from(map.values());
}
function dedupMatieresParNom(matieresArray) {
  const seen = new Set();
  const res = [];
  (matieresArray||[]).forEach(m => {
    const k = (m.nom||'').toLowerCase().trim();
...
function EspaceEnseignant({ eleves, setEleves, classeInitiale }) {
  ...
  const matieresBrutes = matieresParClasse[maClasse] || matieresParClasse[maClasse.toUpperCase()] || matieresParClasse["CM1"] || []; const matieres = (()=>{ const seen=new Set(); return matieresBrutes.filter(m=>{ const k=(m.nom||"").toLowerCase().trim(); if(seen.has(k)) return false; seen.add(k); return true; }); })();
      ...
      {onglet === "matieres" && (
        <Panel title="Matières par Classe - Gestion complète" icon={FileText}>
          ...
          <div className="space-y-2 mb-4">
            {(() => { const arr = matieresParClasse[classeMatiereActive]||[]; const seen=new Set(); const uniq=arr.filter(m=>{ const k=(m.nom||'').toLowerCase().trim(); if(seen.has(k)) return false; seen.add(k); return true; }); return uniq; })().map((m,i)=>(
...
async function syncEleves(eleves) {
  ...
  await pgInsert("frais_eleve", liste.flatMap((e) => dedupFraisParLibelle(e.frais || []).map((f) => ({ eleve_id: e.id, libelle: f.libelle ?? null, du: f.du ?? 0, paye: f.paye ?? 0, mode: f.mode ?? null, date_paiement: f.datePaiement ?? null }))));
