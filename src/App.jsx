...
const supprimerDemande = (demande) => {
  ...
  const supprimerEleve = (eleve) => {
    ...
    marquerElevesSupprimes([eleve.id]);
    setEleves((prev) => {
      const next = prev.filter((e) => e.id !== eleve.id);
      try {
...
async function syncEleves(eleves, options = {}) {
  ...
  // Liste vide : uniquement si clôture d'année (options.vider === true) — sinon on n'efface pas l'école
  if (!liste.length) {
  ...
  // Suppression UNIQUEMENT des IDs explicitement demandés (jamais "tout ce qui n'est pas sur ce téléphone")
  // + IDs persistés localement (survit au refresh / course avec le polling)
  const aSupprimerExplicit = [...new Set([
  ...
  if (aSupprimerExplicit.length) {
    try {
      ...
      // Confirmé en base → on peut retirer de la liste noire locale
      retirerElevesSupprimesConfirmes(aSupprimerExplicit);
    } catch (errDel) {
      console.warn("Suppression eleves (non bloquant):", errDel);
      // On garde les IDs dans la liste noire pour retenter au prochain sync
    }
  }
...
export default function EcoleConnecteeCSP() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      let supprimerIds = [];
      try {
        // Fusion fenêtre + localStorage — ne PAS vider avant le sync (sinon course avec le polling)
      ...
      <div className="flex min-h-screen w-full items-center justify-center px-6" style={{ background: "#FAF6EE" }}>
        <div className="max-w-md text-center">
          ...
          <button
            ...
            className="mt-4 rounded-full px-4 py-2 text-[14px] font-bold"
