...
const CODES_INIT = { directeur: "1234", secretaire: "9012", enseignant: "5678", fondateur: "2026FONDE", enseignants: [{ nom: "Enseignant CI", classe: "CI", code: "ENS-CI-1" }, { nom: "Enseignant CP", classe: "CP", code: "ENS-CP-1" }, { nom: "Enseignant CE1", classe: "CE1", code: "ENS-CE1-1" }, { nom: "Enseignant CE2", classe: "CE2", code: "ENS-CE2-1" }, { nom: "Enseignant CM1", classe: "CM1", code: "ENS-CM1-1" }, { nom: "Enseignant CM2", classe: "CM2", code: "ENS-CM2-1" }, { nom: "Enseignant Maternelle A", classe: "Petite Section", code: "ENS-PS-1" }, { nom: "Enseignant Maternelle B", classe: "Moyenne Section", code: "ENS-MS-1" }] };
...
const ROLES = [
  ...
  { id: "fondateur", label: "Fondateur", icon: BadgeCheck, desc: "Acces total - tous les codes, tout modifier/supprimer" },
];
...
function Connexion({ onEntrer, onInscription, eleves }) {
  ...
  const valider = () => {
    ...
    if (choix === "enseignant") {
      const trouve = (codes.enseignants || []).find((en)=>en.code===saisie);
      if (trouve) { setErreur(""); onEntrer("enseignant", trouve.classe); return; }
      if (saisie === codes.enseignant) { setErreur(""); onEntrer("enseignant", null); return; }
      setErreur("Code enseignant incorrect. Vérifie ton code perso de classe.");
      return;
    }

    if (choix === "fondateur" && saisie === codes.fondateur) { setErreur(""); onEntrer("fondateur"); return; }
...
function EspaceDirecteur({ eleves, personnel, setPersonnel, annonces, setAnnonces, demandes, setDemandes, setEleves }) {
  ...
  const [demandeEnEdition, setDemandeEnEdition] = useState(null); // id en edition
  ...
  const valider = (demande) => {
    ...
    setDemandes((prev) => prev.map((d) => d.id === demande.id ? { ...d, statut: "validee", codeEleve: codeGenere } : d));
    setEleves((prev) => [
      ...prev,
      {
        id: codeGenere,
  ...
  const demarrerEditionDemande = (demande) => {
    setDemandeEnEdition(demande.id);
    setEditionForm({ nom: demande.nom, prenom: demande.prenom, sexe: demande.sexe, naissance: demande.naissance, classeSouhaitee: demande.classeSouhaitee, parrainNom: demande.parrainNom, parrainTelephone: demande.parrainTelephone, message: demande.message||"" });
  };
  const annulerEditionDemande = () => { setDemandeEnEdition(null); setEditionForm({}); };
  const sauvegarderDemande = (demande) => {
    const codeEleve = demande.codeEleve;
    setDemandes((prev)=>prev.map((d)=> d.id===demande.id ? { ...d, nom: editionForm.nom.trim(), prenom: editionForm.prenom.trim(), sexe: editionForm.sexe, naissance: editionForm.naissance, classeSouhaitee: editionForm.classeSouhaitee, parrainNom: editionForm.parrainNom, parrainTelephone: editionForm.parrainTelephone, message: editionForm.message } : d));
    if(demande.statut==="validee"){
      if(codeEleve){
        setEleves((prev)=>prev.map((e)=> e.id===codeEleve ? { ...e, nom: editionForm.nom.trim(), prenom: editionForm.prenom.trim(), classe: editionForm.classeSouhaitee, naissance: editionForm.naissance, sexe: editionForm.sexe, parent: { nom: editionForm.parrainNom, telephone: editionForm.parrainTelephone } } : e));
      } else {
        // fallback ancien systeme sans codeEleve
        const oldNom = demande.nom; const oldPrenom = demande.prenom; const oldClasse = demande.classeSouhaitee;
        setEleves((prev)=>prev.map((e)=> (e.nom===oldNom && e.prenom===oldPrenom && e.classe===oldClasse) ? { ...e, nom: editionForm.nom.trim(), prenom: editionForm.prenom.trim(), classe: editionForm.classeSouhaitee, naissance: editionForm.naissance, sexe: editionForm.sexe, parent: { nom: editionForm.parrainNom, telephone: editionForm.parrainTelephone } } : e));
      }
    }
  ...
  const supprimerDemande = (demande) => {
    if(!confirm(`Supprimer l'inscription de ${demande.prenom} ${demande.nom} ? Si elle était validée, l'élève sera aussi supprimé de la base.`)) return;
    if(demande.statut === "validee"){
      if(demande.codeEleve){
        setEleves((prev) => prev.filter((e) => e.id !== demande.codeEleve));
      } else {
        setEleves((prev) => prev.filter((e) => !(e.nom===demande.nom && e.prenom===demande.prenom && e.classe===demande.classeSouhaitee)));
      }
    }
  ...
  return (
    <Coquille role="directeur" roleLabel="Espace Administratif" nav={nav} actif={onglet} setActif={setOnglet}
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <div className="rounded-lg p-3" style={{background:"#FBF0D2"}}>Frais: <b>{totalPaye}/{totalDu}F</b></div>
                                ...
                                {elevesClasse.map((e)=><div key={e.id} className="border rounded p-2" style={{borderColor:"#E7DEC8"}}><div className="font-bold">{e.prenom} {e.nom} - {e.id}</div><div className="text-[13px]">Notes: {e.notes.length} | Absences: {e.absences.length} | Paye: {e.frais.reduce((a,f)=>a+f.paye,0)}F</div><div className="text-[12px]" style={{color:"#5C5240"}}>Parent: {e.parent?.nom} ({e.parent?.telephone}) | Classe: {e.classe}</div><div className="mt-1 flex gap-2"><button onClick={()=>{ const n=prompt("Nouveau nom", e.nom); const p=prompt("Nouveau prenom", e.prenom); const cl=prompt("Nouvelle classe", e.classe); if(n!==null||p!==null||cl!==null){ setEleves((prev)=>prev.map((el)=>el.id===e.id?{...el, nom: n!==null&&n.trim()?n.trim():el.nom, prenom: p!==null&&p.trim()?p.trim():el.prenom, classe: cl!==null&&cl.trim()?cl.trim():el.classe}:el)); } }} className="px-2 py-1 rounded text-[11px]" style={{background:"#1B2A4A", color:"white"}}>Modifier</button><button onClick={()=>{ if(confirm(`Supprimer ${e.prenom} ${e.nom}?`)){ setEleves((prev)=>prev.filter((el)=>el.id!==e.id)); } }} className="px-2 py-1 rounded text-[11px]" style={{background:"#8A2E2E", color:"white"}}>Supprimer</button></div></div>)}
              </div>
            );
          })()}
        </Panel>
          ...
          <div className="space-y-3">
            ...
            {demandes.map((d) => (
              <div key={d.id} className="rounded-lg border p-4" style={{ borderColor: "#E7DEC8" }}>
                ...
                {d.statut === "en_attente" && demandeEnEdition!==d.id && (
                  <div className="mt-3 flex gap-2 flex-wrap">
                    ...
                    <Btn variant="ghost" icon={Pencil} small onClick={()=>demarrerEditionDemande(d)}>✏️ Modifier</Btn>
                    <Btn variant="ghost" icon={Trash2} small onClick={() => supprimerDemande(d)}>Supprimer</Btn>
                  </div>
                )}
                {demandeEnEdition===d.id && (
                  <div className="mt-3 space-y-2 rounded-lg border p-3" style={{borderColor:"#C9A227", background:"#FFFDF6"}}>
                    <div className="grid grid-cols-2 gap-2">
                      <input value={editionForm.prenom} onChange={(e)=>setEditionForm({...editionForm, prenom:e.target.value})} placeholder="Prénom" className="border rounded px-2 py-1 text-[13px]" />
                      <input value={editionForm.nom} onChange={(e)=>setEditionForm({...editionForm, nom:e.target.value})} placeholder="Nom" className="border rounded px-2 py-1 text-[13px]" />
                      <select value={editionForm.sexe} onChange={(e)=>setEditionForm({...editionForm, sexe:e.target.value})} className="border rounded px-2 py-1 text-[13px]"><option value="M">Masculin</option><option value="F">Féminin</option></select>
                      <input type="date" value={editionForm.naissance} onChange={(e)=>setEditionForm({...editionForm, naissance:e.target.value})} className="border rounded px-2 py-1 text-[13px]" />
                      <select value={editionForm.classeSouhaitee} onChange={(e)=>setEditionForm({...editionForm, classeSouhaitee:e.target.value})} className="border rounded px-2 py-1 text-[13px]">{classes.map((cl)=><option key={cl} value={cl}>{cl}</option>)}</select>
                      <input value={editionForm.parrainNom} onChange={(e)=>setEditionForm({...editionForm, parrainNom:e.target.value})} placeholder="Parrain Nom" className="border rounded px-2 py-1 text-[13px]" />
                      <input value={editionForm.parrainTelephone} onChange={(e)=>setEditionForm({...editionForm, parrainTelephone:e.target.value})} placeholder="Parrain Tel" className="border rounded px-2 py-1 text-[13px]" />
                    </div>
                    <textarea value={editionForm.message} onChange={(e)=>setEditionForm({...editionForm, message:e.target.value})} placeholder="Message" className="w-full border rounded px-2 py-1 text-[13px]" />
                    <div className="flex gap-2">
                      <Btn variant="gold" small onClick={()=>sauvegarderDemande(d)}>💾 Enregistrer</Btn>
                      <Btn variant="ghost" small onClick={annulerEditionDemande}>Annuler</Btn>
                    </div>
                  </div>
                )}
                {demandeEnEdition!==d.id && d.statut!=="en_attente" && (
                  <div className="mt-3 flex gap-2 flex-wrap">
                    <Btn variant="ghost" icon={Pencil} small onClick={()=>demarrerEditionDemande(d)}>✏️ Modifier {d.statut==="validee" ? "(inscription + élève)" : ""}</Btn>
                    <Btn variant="ghost" icon={Trash2} small onClick={() => supprimerDemande(d)}>🗑️ Supprimer {d.statut==="validee" ? "(+ élève)" : ""}</Btn>
                  </div>
                )}
      ...
      {onglet === "annonces" && (
        <Panel title="Annonces de l'ecole" icon={Megaphone}>
          <p className="mb-3 text-[13px]" style={{color:"#9A8B67"}}>Toutes les annonces publiees par le Directeur et le Secretaire. Tu recois tout ici.</p>
          <div className="space-y-2">
            {annonces.length===0 && <p className="text-[15px]" style={{color:"#9A8B67"}}>Aucune annonce pour le moment.</p>}
            {annonces.map((a,i)=>(
              <div key={i} className="rounded-lg border px-4 py-3" style={{borderColor:"#E7DEC8", background:"#FFFDF6"}}>
                <div className="flex items-center justify-between">
                  <span className="font-bold text-[15px]" style={{color:"#1B2A4A"}}>{a.titre}</span>
                  <span className="text-[12px]" style={{color:"#9A8B67"}}>{a.date}</span>
                </div>
                <p className="mt-1 text-[15px]" style={{color:"#3E3625"}}>{a.texte}</p>
              </div>
            ))}
          </div>
        </Panel>
      ...
      {onglet === "archives" && (
        <Panel title="Archives Annees Scolaires" icon={CalendarClock}>
          <div className="space-y-2">
            {anneesArchivees.length===0 && <p className="text-[15px]" style={{color:"#9A8B67"}}>Aucune archive.</p>}
            {anneesArchivees.map((ar,i)=>(
              <div key={i} className="border rounded p-3" style={{borderColor:"#E7DEC8"}}>
                <div className="font-bold">{ar.annee} - Archive du {ar.dateArchivage}</div>
                <div className="text-[13px]">{ar.eleves?.length||0} eleves archives</div>
              </div>
            ))}
          </div>
        </Panel>
      ...
      {onglet === "messagesParents" && (
        <Panel title="Messages reçus des parents" icon={MessageSquare}>
          ...
          <div className="space-y-3">
            ...
            {onglet === "parametres" && (
        <Panel title="Paramètres - 8 Enseignants - Codes Perso par Classe" icon={Settings}>
          <div className="mb-6">
            <h4 className="font-bold mb-2">Codes Enseignants (8 classes) - Partage 1 lien + 1 code par enseignant</h4>
            <p className="text-[13px] mb-3" style={{color:"#9A8B67"}}>Chaque enseignant clique sur le meme lien https://csp-sagesse-divine.vercel.app puis Espace Enseignant puis tape SON code perso. Il rentre direct dans SA classe, pas les autres.</p>
            <div className="space-y-2">
              {(codes.enseignants || []).map((ens, idx)=>(
                <div key={idx} className="flex items-center gap-2 border rounded p-2" style={{borderColor:"#E7DEC8"}}>
                  <span className="w-32 font-bold">{ens.classe}</span>
                  <span className="flex-1">{ens.nom}</span>
                  <code className="bg-[#F1ECDD] px-2 py-1 rounded">{ens.code}</code>
                  <button onClick={()=>setCodes((prev)=>({...prev, enseignants: prev.enseignants.filter((_,i)=>i!==idx)}))} className="text-red-600"><Trash2 size={14}/></button>
                </div>
              ))}
            </div>
            <div className="mt-3 flex gap-2">
              <input id="newEnsNom" placeholder="Nom ex: M. Koffi" className="border rounded px-2 py-1 text-[13px]" />
              <select id="newEnsClasse" className="border rounded px-2 py-1 text-[13px]">{classes.map((cl)=><option key={cl} value={cl}>{cl}</option>)}</select>
              <input id="newEnsCode" placeholder="Code ex: ENS-CI-2" className="border rounded px-2 py-1 text-[13px]" />
              <button onClick={()=>{ const nom=document.getElementById("newEnsNom").value.trim()||"Enseignant"; const classe=document.getElementById("newEnsClasse").value; const code=document.getElementById("newEnsCode").value.trim(); if(!code) return; setCodes((prev)=>({...prev, enseignants:[...prev.enseignants, {nom, classe, code}]})); document.getElementById("newEnsNom").value=""; document.getElementById("newEnsCode").value=""; }} className="bg-[#1B2A4A] text-white px-3 py-1 rounded text-[13px]"><Plus size={12}/> Ajouter</button>
            </div>
          </div>
          <div className="border-t pt-4 mt-6" />
          <div className="mt-6 rounded-lg border-2 border-red-300 p-4" style={{background:"#FFF1F1"}}>
            <h4 className="font-bold text-red-700 mb-2">⚠️ Remise à zéro - Vider la base</h4>
            <p className="text-[13px] mb-3">Tu veux tout effacer pour ajouter toi-même tes vraies informations ? Clique ci-dessous. Ça va vider tous les élèves, personnel, annonces, messages, frais, demandes. Tes codes d'accès Directeur restent.</p>
            <div className="flex gap-2">
              <button onClick={()=>{ if(!confirm("Vraiment vider TOUS les élèves et messages ? Cette action efface Supabase et est irréversible !")) return; setEleves([]); setAnnonces([]); setDemandes([]); setAnneesArchivees([]); setMessagesInternes([]); setMessagesParents([]); alert("Base vidée ! Maintenant ajoute tes classes, matières, frais, élèves."); }} className="bg-red-600 text-white px-4 py-2 rounded text-[13px]">🗑️ Vider Élèves + Messages + Annonces</button>
              <button onClick={()=>{ if(!confirm("Remise à zéro TOTALE ? Ça va aussi vider personnel, classes, matières, frais. Codes directeur gardés.")) return; setEleves([]); setAnnonces([]); setDemandes([]); setAnneesArchivees([]); setMessagesInternes([]); setMessagesParents([]); setPersonnel([]); setFraisTypes([]); setClasses([]); setMatieresParClasse({}); alert("Remise à zéro totale faite ! Ajoute maintenant tes vraies données."); }} className="bg-[#1B2A4A] text-white px-4 py-2 rounded text-[13px]">🔄 Remise à zéro TOTALE</button>
            </div>
          </div>
        </Panel>
      )}
      {false && (
        <Panel title="Paramètres - Codes d'accès" icon={Settings}>{onglet === "parametres_OLD" && (
        <>
...
function EspaceEnseignant({ eleves, setEleves, classeInitiale }) {
  const { matieresParClasse, setMatieresParClasse, classes, setMessagesInternes, codes } = useEcole();
  const [onglet, setOnglet] = useState("classe");
  const [maClasse, setMaClasse] = useState(classeInitiale || codes.enseignants?.[0]?.classe || classes[0] || "CM1");
  const enseignantActuel = codes.enseignants?.find((e)=>e.classe===maClasse) || { nom: "Enseignant " + maClasse, classe: maClasse };
  const estClasseFixe = !!classeInitiale;
...
/* ------------------------------------------------------------------
   ESPACE FONDATEUR - ACCES TOTAL SUPER ADMIN
   - Voir tout
   - Modifier/supprimer tout (eleves, personnel, classes, matieres, frais, annonces, demandes, messages)
   - Changer tous les codes : directeur, secretaire, fondateur, 8 enseignants
------------------------------------------------------------------- */


function EspaceFondateur({ eleves, personnel, setPersonnel, annonces, setAnnonces, demandes, setDemandes, setEleves }) {
  const { ecole, setEcole, classes, setClasses, codes, setCodes, anneeScolaire, setAnneeScolaire, fraisTypes, setFraisTypes, matieresParClasse, setMatieresParClasse, messagesInternes, setMessagesInternes, anneesArchivees, setAnneesArchivees, messagesParents, setMessagesParents } = useEcole();
  const [onglet, setOnglet] = useState("dashboard");
  const [searchEleve, setSearchEleve] = useState("");
  const [editEleve, setEditEleve] = useState(null);

  const nav = [
    { id: "dashboard", label: "Tableau de bord Fondateur", icon: LayoutDashboard },
    { id: "codes", label: "Tous les Codes (SUPER ADMIN)", icon: KeyRound },
    { id: "eleves", label: "Tous les Eleves - Modifier/Supprimer", icon: Users },
    { id: "demandes", label: "Inscriptions & Demandes", icon: Inbox },
    { id: "personnel", label: "Personnel - Tout modifier", icon: Users },
    { id: "classes", label: "Classes & Matieres", icon: School },
    { id: "frais", label: "Frais Scolaires", icon: Wallet },
    { id: "annonces", label: "Annonces & Messages", icon: Megaphone },
    { id: "parametres", label: "Parametres Ecole + RESET", icon: Settings },
  ];
  ...
  const stats = {
    ...
    totalEnAttente: demandes.filter(d=>d.statut==="en_attente").length,
  };

  const elevesFiltres = eleves.filter(e=> 
    !searchEleve || `${e.prenom} ${e.nom} ${e.id} ${e.classe}`.toLowerCase().includes(searchEleve.toLowerCase())
  );

  return (
    <Coquille role="fondateur" roleLabel="ESPACE FONDATEUR - ACCES TOTAL ABSOLU" nav={nav} actif={onglet} setActif={setOnglet} onQuitter={()=>{}}>
      {onglet==="dashboard" && (
        <>
          <div className="rounded-xl p-4 mb-4" style={{background:"#1B2A4A", color:"white"}}>
            <h2 className="font-serif text-xl flex items-center gap-2"><ShieldCheck size={20} color="#C9A227"/> Bienvenue Fondateur - Tu as la main totale</h2>
            <p className="text-[13px] mt-1 opacity-80">Tu peux voir TOUT, modifier TOUT, supprimer TOUT. Tu changes les codes de Directeur, Secrétaire, Enseignants, et ton propre code Fondateur. Tu es le super-administrateur de la plateforme.</p>
          </div>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            <Stat label="Elèves" value={stats.totalEleves} tone="ardoise" />
            <Stat label="Personnel" value={stats.totalPersonnel} tone="ardoise" />
            <Stat label="Classes" value={stats.totalClasses} tone="or" />
            <Stat label="Demandes en attente" value={stats.totalEnAttente} tone="or" />
            <Stat label="Annonces" value={stats.totalAnnonces} tone="ardoise" />
            <Stat label="Total Demandes" value={stats.totalDemandes} tone="ardoise" />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
            <Panel title="Actions Rapides Fondateur" icon={BadgeCheck}>
              <div className="grid grid-cols-1 gap-2">
                <button onClick={()=>setOnglet("codes")} className="flex items-center gap-2 bg-[#C9A227] text-[#1B2A4A] px-3 py-2 rounded font-bold"><KeyRound size={14}/> Changer code Directeur / Secrétaire / Fondateur</button>
                <button onClick={()=>setOnglet("eleves")} className="flex items-center gap-2 bg-[#1B2A4A] text-white px-3 py-2 rounded"><Users size={14}/> Voir & Modifier tous les élèves</button>
                <button onClick={()=>setOnglet("demandes")} className="flex items-center gap-2 bg-white border px-3 py-2 rounded" style={{borderColor:"#E7DEC8"}}><Inbox size={14}/> Valider / Modifier / Supprimer demandes</button>
              </div>
            </Panel>
            <Panel title="Sécurité" icon={ShieldCheck}>
              <p className="text-[13px]" style={{color:"#5C5240"}}>Code Fondateur actuel: <span className="font-mono font-bold">{codes.fondateur}</span> - Ne le partage jamais. Seul toi peux changer les codes d'administration.</p>
              <div className="mt-3 p-2 rounded" style={{background:"#FFF1F1"}}>
                <p className="text-[12px] text-red-700">⚠️ Tu es le seul à pouvoir supprimer toute la plateforme, vider la base, changer tous les codes, y compris celui du Directeur (1234). Utilise avec prudence.</p>
              </div>
            </Panel>
          </div>
        </>
      ...
      {onglet==="codes" && (
        <Panel title="SUPER ADMIN - Tous les Codes d'Accès (Directeur, Secrétaire, Fondateur, Enseignants)" icon={KeyRound}>
          <div className="rounded-lg p-3 mb-4" style={{background:"#FBF0D2", border:"1px solid #C9A227"}}>
            <p className="text-[13px] font-bold" style={{color:"#8A6D14"}}>🔑 Ici tu as la MAIN TOTALE : tu peux changer le code de l'administration (Directeur), du Secrétariat, du Fondateur (toi), et de chaque enseignant.</p>
          </div>
          <div className="space-y-4">
            <div className="border rounded p-4" style={{borderColor:"#1B2A4A", background:"#F8F6F0"}}>
              <h4 className="font-bold mb-3 flex items-center gap-2"><ShieldCheck size={16}/> Codes Administration - Modifiables par toi seul</h4>
              <div className="space-y-3">
                <div className="flex flex-col sm:flex-row gap-2 sm:items-center">
                  <span className="w-40 font-medium text-[14px]">👑 Directeur (Administration)</span>
                  <input value={codes.directeur} onChange={(e)=>setCodes({...codes, directeur:e.target.value})} className="border rounded px-3 py-2 flex-1 font-mono bg-white" placeholder="Nouveau code directeur" />
                  <span className="text-[11px] px-2 py-1 rounded" style={{background:"#1B2A4A", color:"white"}}>ACTUEL: {codes.directeur}</span>
                </div>
                <div className="flex flex-col sm:flex-row gap-2 sm:items-center">
                  <span className="w-40 font-medium text-[14px]">📋 Secrétaire</span>
                  <input value={codes.secretaire} onChange={(e)=>setCodes({...codes, secretaire:e.target.value})} className="border rounded px-3 py-2 flex-1 font-mono bg-white" />
                  <span className="text-[11px] px-2 py-1 rounded" style={{background:"#E7DEC8"}}>ACTUEL: {codes.secretaire}</span>
                </div>
                <div className="flex flex-col sm:flex-row gap-2 sm:items-center">
                  <span className="w-40 font-bold text-[14px]">🔐 FONDATEUR (TOI)</span>
                  <input value={codes.fondateur} onChange={(e)=>setCodes({...codes, fondateur:e.target.value})} className="border-2 rounded px-3 py-2 flex-1 font-mono bg-yellow-50" style={{borderColor:"#C9A227"}} />
                  <span className="text-[11px] px-2 py-1 rounded bg-red-600 text-white">TON CODE SECRET</span>
                </div>
                <div className="flex flex-col sm:flex-row gap-2 sm:items-center">
                  <span className="w-40 font-medium text-[14px]">👨‍🏫 Enseignant Générique</span>
                  <input value={codes.enseignant} onChange={(e)=>setCodes({...codes, enseignant:e.target.value})} className="border rounded px-3 py-2 flex-1 font-mono bg-white" />
                </div>
              </div>
            </div>
            <div className="border rounded p-4" style={{borderColor:"#E7DEC8"}}>
              <h4 className="font-bold mb-3">👨‍🏫 Codes Enseignants Personnels (1 par classe) - Tu peux tout changer</h4>
              <div className="space-y-2">
                {(codes.enseignants||[]).map((ens,idx)=>(
                  <div key={idx} className="flex flex-col sm:flex-row gap-2 items-start sm:items-center p-2 rounded" style={{background:"#FAF6EE"}}>
                    <input value={ens.nom} onChange={(e)=>{ const arr=[...codes.enseignants]; arr[idx].nom=e.target.value; setCodes({...codes, enseignants:arr}); }} className="border rounded px-2 py-1.5 w-full sm:w-36 text-[13px]" placeholder="Nom enseignant" />
                    <select value={ens.classe} onChange={(e)=>{ const arr=[...codes.enseignants]; arr[idx].classe=e.target.value; setCodes({...codes, enseignants:arr}); }} className="border rounded px-2 py-1.5 w-full sm:w-28 text-[13px]">{classes.map((cl)=><option key={cl} value={cl}>{cl}</option>)}</select>
                    <input value={ens.code} onChange={(e)=>{ const arr=[...codes.enseignants]; arr[idx].code=e.target.value; setCodes({...codes, enseignants:arr}); }} className="border rounded px-2 py-1.5 flex-1 font-mono text-[13px] bg-white" placeholder="Code" />
                    <button onClick={()=>{ if(confirm("Supprimer ce code enseignant?")) setCodes({...codes, enseignants: codes.enseignants.filter((_,i)=>i!==idx)}); }} className="bg-red-600 text-white px-2 py-1 rounded text-[12px]">Supprimer</button>
                  </div>
                ))}
              </div>
              <button onClick={()=>setCodes({...codes, enseignants:[...(codes.enseignants||[]), {nom:"Nouvel Enseignant", classe:classes[0]||"CI", code:"ENS-"+Math.random().toString(36).slice(2,6).toUpperCase()}]})} className="mt-3 bg-[#1B2A4A] text-white px-4 py-2 rounded text-[13px] flex items-center gap-1"><Plus size={14}/> Ajouter un enseignant + son code</button>
            </div>
          </div>
        </Panel>
      )}
      ...
      {onglet==="eleves" && (
        <Panel title="Tous les Elèves - Tu peux tout Modifier & Supprimer" icon={Users}>
          <div className="mb-3 flex gap-2">
            <input value={searchEleve} onChange={(e)=>setSearchEleve(e.target.value)} placeholder="Rechercher nom, classe, code..." className="border rounded px-3 py-2 flex-1" style={{borderColor:"#E7DEC8"}} />
            <span className="px-3 py-2 rounded text-[13px]" style={{background:"#F1ECDD"}}>{elevesFiltres.length} élèves</span>
          </div>
          <div className="space-y-2 max-h-[60vh] overflow-y-auto">
            {elevesFiltres.map((e)=>(
              <div key={e.id} className="border rounded p-3 flex flex-col sm:flex-row justify-between gap-2" style={{borderColor: editEleve?.id===e.id ? "#C9A227" : "#E7DEC8", background: editEleve?.id===e.id ? "#FFFDF6" : "white"}}>
                {editEleve?.id!==e.id ? (
                  <>
                    <div className="flex-1">
                      <div className="font-bold">{e.prenom} {e.nom} - <span className="font-mono text-[12px]">{e.id}</span> - {e.classe}</div>
                      <div className="text-[12px]" style={{color:"#5C5240"}}>Né: {e.naissance||"—"} | Sexe: {e.sexe||"—"} | Parent: {e.parent?.nom} ({e.parent?.telephone}) | Frais payés: {e.frais?.reduce((a,f)=>a+f.paye,0)||0}F</div>
                    </div>
                    <div className="flex gap-1">
                      <button onClick={()=>setEditEleve(e)} className="bg-[#1B2A4A] text-white px-3 py-1 rounded text-[12px]">✏️ Modifier</button>
                      <button onClick={()=>{ if(confirm(`Supprimer définitivement ${e.prenom} ${e.nom} ?`)){ setEleves(prev=>prev.filter(el=>el.id!==e.id)); setDemandes(prev=>prev.map(d=>d.codeEleve===e.id?{...d, statut:"supprimee"}:d)); } }} className="bg-red-600 text-white px-3 py-1 rounded text-[12px]">🗑️ Supprimer</button>
                    </div>
                  </>
                ) : (
                  <div className="w-full space-y-2">
                    <div className="grid grid-cols-2 gap-2">
                      <input value={editEleve.prenom} onChange={(ev)=>setEditEleve({...editEleve, prenom:ev.target.value})} placeholder="Prénom" className="border rounded px-2 py-1 text-[13px]" />
                      <input value={editEleve.nom} onChange={(ev)=>setEditEleve({...editEleve, nom:ev.target.value})} placeholder="Nom" className="border rounded px-2 py-1 text-[13px]" />
                      <input value={editEleve.naissance||""} type="date" onChange={(ev)=>setEditEleve({...editEleve, naissance:ev.target.value})} className="border rounded px-2 py-1 text-[13px]" />
                      <select value={editEleve.sexe||"M"} onChange={(ev)=>setEditEleve({...editEleve, sexe:ev.target.value})} className="border rounded px-2 py-1 text-[13px]"><option value="M">Masculin</option><option value="F">Féminin</option></select>
                      <select value={editEleve.classe} onChange={(ev)=>setEditEleve({...editEleve, classe:ev.target.value})} className="border rounded px-2 py-1 text-[13px]">{classes.map(cl=><option key={cl} value={cl}>{cl}</option>)}</select>
                      <input value={editEleve.parent?.nom||""} onChange={(ev)=>setEditEleve({...editEleve, parent:{...editEleve.parent, nom:ev.target.value}})} placeholder="Nom parent" className="border rounded px-2 py-1 text-[13px]" />
                      <input value={editEleve.parent?.telephone||""} onChange={(ev)=>setEditEleve({...editEleve, parent:{...editEleve.parent, telephone:ev.target.value}})} placeholder="Tél parent" className="border rounded px-2 py-1 text-[13px]" />
                    </div>
                    <div className="flex gap-2">
                      <button onClick={()=>{ setEleves(prev=>prev.map(el=>el.id===editEleve.id?{...el, prenom:editEleve.prenom, nom:editEleve.nom, classe:editEleve.classe, naissance:editEleve.naissance, sexe:editEleve.sexe, parent:editEleve.parent}:el)); setDemandes(prev=>prev.map(d=>d.codeEleve===editEleve.id?{...d, prenom:editEleve.prenom, nom:editEleve.nom, classeSouhaitee:editEleve.classe}:d)); setEditEleve(null); }} className="bg-[#C9A227] text-[#1B2A4A] px-3 py-1 rounded text-[13px] font-bold">💾 Enregistrer</button>
                      <button onClick={()=>setEditEleve(null)} className="border px-3 py-1 rounded text-[13px]">Annuler</button>
                    </div>
                  </div>
                )}
              </div>
      ...
      {onglet==="demandes" && (
        <Panel title="Demandes d'Inscription - Fondateur peut tout faire" icon={Inbox}>
          <div className="space-y-2">
            {demandes.map((d,i)=>(
              <div key={d.id||i} className="border rounded p-3" style={{borderColor:"#E7DEC8"}}>
                <div className="flex justify-between"><b>{d.prenom} {d.nom}</b><span className="text-[12px] px-2 py-0.5 rounded" style={{background: d.statut==="en_attente"?"#FBF0D2": d.statut==="validee"?"#E4EEE3":"#EEE"}}>{d.statut}</span></div>
                <div className="text-[13px]">Classe: {d.classeSouhaitee} | Parent: {d.parrainNom} ({d.parrainTelephone}) | Code: {d.codeEleve||"—"}</div>
                <div className="flex gap-1 mt-2 flex-wrap">
                  {d.statut==="en_attente" && <button onClick={()=>{ const codeGenere="CSP"+Math.floor(1000+Math.random()*9000); const nouvelEleve={id:codeGenere, prenom:d.prenom, nom:d.nom, classe:d.classeSouhaitee, naissance:d.naissance, sexe:d.sexe, parent:{nom:d.parrainNom, telephone:d.parrainTelephone}, notes:[], absences:[], frais:fraisTypes.map(ft=>({libelle:ft.libelle, du:ft.montant, paye:0})), enseignant:"Non assigné"}; setEleves(prev=>[nouvelEleve,...prev]); setDemandes(prev=>prev.map(x=>x.id===d.id?{...x, statut:"validee", codeEleve:codeGenere}:x)); }} className="bg-green-600 text-white px-2 py-1 rounded text-[12px]">✅ Valider</button>}
                  <button onClick={()=>{ if(confirm("Supprimer cette demande?")) setDemandes(prev=>prev.filter((_,idx)=>idx!==i)); }} className="bg-red-600 text-white px-2 py-1 rounded text-[12px]">🗑️ Supprimer demande</button>
                  {d.codeEleve && <button onClick={()=>{ if(confirm("Supprimer aussi l'élève lié?")){ setEleves(prev=>prev.filter(el=>el.id!==d.codeEleve)); setDemandes(prev=>prev.filter((_,idx)=>idx!==i)); } }} className="border px-2 py-1 rounded text-[12px]">Supprimer + élève</button>}
                </div>
              </div>
            ))}
          </div>
      ...
      {onglet==="personnel" && (
        <Panel title="Personnel - Fondateur modifie & supprime tout" icon={Users}>
          <div className="space-y-2">
            {personnel.map((p,i)=>(
              <div key={i} className="border rounded p-2 flex justify-between items-center" style={{borderColor:"#E7DEC8"}}>
                <span className="text-[13px]"><b>{p.nom}</b> - {p.poste} - {p.classe} - {p.telephone}</span>
                <div className="flex gap-1">
                  <button onClick={()=>{ const newNom=prompt("Nouveau nom", p.nom); if(newNom) setPersonnel(prev=>prev.map((_,idx)=>idx===i?{...p, nom:newNom}:_)); }} className="bg-[#1B2A4A] text-white px-2 py-1 rounded text-[11px]">Modifier</button>
                  <button onClick={()=>{ if(confirm("Supprimer ce personnel?")) setPersonnel(prev=>prev.filter((_,idx)=>idx!==i)); }} className="bg-red-600 text-white px-2 py-1 rounded text-[11px]">Supprimer</button>
                </div>
              </div>
            ))}
            <button onClick={()=>{ const nom=prompt("Nom du personnel"); if(!nom) return; const poste=prompt("Poste (Enseignant, Directeur, etc)", "Enseignant"); const cl=prompt("Classe", classes[0]); setPersonnel(prev=>[...prev, {nom, poste:poste||"Enseignant", classe:cl||classes[0], telephone:""}]); }} className="bg-[#C9A227] text-[#1B2A4A] px-3 py-1 rounded text-[13px] font-bold">+ Ajouter personnel</button>
          </div>
        </Panel>
      )}

      {onglet==="classes" && (
        <Panel title="Classes & Matières - Tout gérer" icon={School}>
          <div className="mb-3">
            <div className="flex gap-2 flex-wrap mb-2">{classes.map((cl,idx)=><div key={cl} className="border rounded px-3 py-1 flex gap-2 items-center bg-white">{cl}<button onClick={()=>{ if(confirm("Supprimer classe "+cl+" ?")){ setClasses(prev=>prev.filter((_,i)=>i!==idx)); const copy={...matieresParClasse}; delete copy[cl]; setMatieresParClasse(copy); } }} className="text-red-600"><Trash2 size={12}/></button></div>)}</div>
            <div className="flex gap-2"><input id="newClasse" placeholder="Nouvelle classe (ex: 6ème)" className="border rounded px-2 py-1 flex-1" /><button onClick={()=>{ const inp=document.getElementById("newClasse"); const v=inp.value.trim(); if(!v) return; if(classes.includes(v)){ alert("Classe existe déjà"); return; } setClasses(prev=>[...prev, v]); setMatieresParClasse(prev=>({...prev, [v]:[{nom:"Français", coef:2}]})); inp.value=""; }} className="bg-[#1B2A4A] text-white px-3 py-1 rounded">Ajouter classe</button></div>
          </div>
          <div className="space-y-3">
            {Object.entries(matieresParClasse).map(([cl, mats])=>(
              <div key={cl} className="border rounded p-3" style={{borderColor:"#E7DEC8"}}><div className="flex justify-between"><b>{cl}</b><button onClick={()=>{ const mat=prompt("Nouvelle matière pour "+cl); if(!mat) return; setMatieresParClasse(prev=>({...prev, [cl]:[...(prev[cl]||[]), {nom:mat, coef:1}]})); }} className="text-[12px] bg-[#F1ECDD] px-2 py-0.5 rounded">+ Matière</button></div><div className="flex gap-1 flex-wrap mt-2">{mats.map((m,mi)=><span key={mi} className="text-[12px] border rounded px-2 py-1 flex items-center gap-1">{m.nom} ({m.coef}) <button onClick={()=>setMatieresParClasse(prev=>({...prev, [cl]:prev[cl].filter((_,i)=>i!==mi)}))} className="text-red-600"><XCircle size={10}/></button></span>)}</div></div>
            ))}
          </div>
        </Panel>
      )}
      ...
      {onglet==="frais" && (
        <Panel title="Frais Scolaires - Gérés par Fondateur" icon={Wallet}>
          <div className="space-y-2">
            {fraisTypes.map((f,i)=><div key={i} className="border rounded p-3 flex justify-between items-center"><div><b>{f.libelle}</b> - {f.montant}F</div><div className="flex gap-1"><button onClick={()=>{ const v=prompt("Nouveau montant pour "+f.libelle, f.montant); if(v){ setFraisTypes(prev=>prev.map((_,idx)=>idx===i?{...f, montant:parseInt(v)||0}:_)); } }} className="bg-[#1B2A4A] text-white px-2 py-1 rounded text-[11px]">Modifier montant</button><button onClick={()=>{ if(confirm("Supprimer ce frais?")) setFraisTypes(prev=>prev.filter((_,idx)=>idx!==i)); }} className="bg-red-600 text-white px-2 py-1 rounded text-[11px]">Supprimer</button></div></div>)}
            <button onClick={()=>{ const lib=prompt("Libellé frais (ex: Cantine)"); if(!lib) return; const mont=prompt("Montant", "10000"); setFraisTypes(prev=>[...prev, {libelle:lib, montant:parseInt(mont)||0}]); }} className="bg-[#C9A227] text-[#1B2A4A] px-3 py-2 rounded font-bold">+ Ajouter frais</button>
          </div>
        </Panel>
      )}

      {onglet==="annonces" && (
        <Panel title="Annonces & Messages - Contrôle Total" icon={Megaphone}>
          <div className="mb-4">
            <h4 className="font-bold mb-2">Annonces Ecole</h4>
            {annonces.map((a,i)=><div key={i} className="border rounded p-2 mb-2 flex justify-between gap-2"><div><b className="text-[13px]">{a.titre}</b><p className="text-[12px]">{a.texte}</p><span className="text-[11px]" style={{color:"#9A8B67"}}>{a.date}</span></div><button onClick={()=>{ if(confirm("Supprimer annonce?")) setAnnonces(prev=>prev.filter((_,idx)=>idx!==i)); }} className="bg-red-600 text-white px-2 py-1 rounded text-[11px] h-fit">Supprimer</button></div>)}
            <div className="flex gap-2 mt-3"><input id="titreAnnonce" placeholder="Titre annonce" className="border rounded px-2 py-1 flex-1" /><input id="texteAnnonce" placeholder="Texte" className="border rounded px-2 py-1 flex-1" /><button onClick={()=>{ const t=document.getElementById("titreAnnonce").value; const txt=document.getElementById("texteAnnonce").value; if(!t||!txt) return; setAnnonces(prev=>[{titre:t, texte:txt, date:new Date().toLocaleDateString()}, ...prev]); document.getElementById("titreAnnonce").value=""; document.getElementById("texteAnnonce").value=""; }} className="bg-[#1B2A4A] text-white px-3 py-1 rounded">Publier</button></div>
          </div>
          <div className="border-t pt-4">
            <h4 className="font-bold mb-2">Messages Parents ({messagesParents.length})</h4>
            {messagesParents.map((m,i)=><div key={i} className="border rounded p-2 mb-1 text-[13px] flex justify-between"><span><b>{m.nomEleve||m.eleveId}</b> ({m.classe}): {m.texte}</span><button onClick={()=>setMessagesParents(prev=>prev.filter((_,idx)=>idx!==i))} className="text-red-600"><Trash2 size={12}/></button></div>)}
            <h4 className="font-bold mt-4 mb-2">Messages Internes ({messagesInternes.length})</h4>
            {messagesInternes.map((m,i)=><div key={i} className="border rounded p-2 mb-1 text-[13px] flex justify-between"><span>À {m.destinataire}: {m.texte}</span><button onClick={()=>setMessagesInternes(prev=>prev.filter((_,idx)=>idx!==i))} className="text-red-600"><Trash2 size={12}/></button></div>)}
          </div>
        </Panel>
      )}

      {onglet==="parametres" && (
        <Panel title="Paramètres Ecole - Fondateur a main totale + RESET" icon={Settings}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div><label className="text-[12px]">Nom école</label><input value={ecole.nom} onChange={(e)=>setEcole({...ecole, nom:e.target.value})} className="border rounded px-2 py-1.5 w-full" /></div>
            <div><label className="text-[12px]">Sigle</label><input value={ecole.sigle} onChange={(e)=>setEcole({...ecole, sigle:e.target.value})} className="border rounded px-2 py-1.5 w-full" /></div>
            <div><label className="text-[12px]">Quartier</label><input value={ecole.quartier} onChange={(e)=>setEcole({...ecole, quartier:e.target.value})} className="border rounded px-2 py-1.5 w-full" /></div>
            <div><label className="text-[12px]">Téléphone école</label><input value={ecole.telephone} onChange={(e)=>setEcole({...ecole, telephone:e.target.value})} className="border rounded px-2 py-1.5 w-full" /></div>
            <div><label className="text-[12px]">Tél Directeur</label><input value={ecole.telephoneDirecteur} onChange={(e)=>setEcole({...ecole, telephoneDirecteur:e.target.value})} className="border rounded px-2 py-1.5 w-full" /></div>
            <div><label className="text-[12px]">Email</label><input value={ecole.email} onChange={(e)=>setEcole({...ecole, email:e.target.value})} className="border rounded px-2 py-1.5 w-full" /></div>
            <div><label className="text-[12px]">Année scolaire</label><input value={anneeScolaire} onChange={(e)=>setAnneeScolaire(e.target.value)} className="border rounded px-2 py-1.5 w-full font-mono" /></div>
            <div><label className="text-[12px]">Devise</label><input value={ecole.devise} onChange={(e)=>setEcole({...ecole, devise:e.target.value})} className="border rounded px-2 py-1.5 w-full" /></div>
          </div>
          <div className="mt-6 border-t pt-4 space-y-3">
            <h4 className="font-bold text-red-600 flex items-center gap-2"><Trash2 size={16}/> Zone DANGER - Pouvoir Absolu Fondateur</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <button onClick={()=>{ if(!confirm("Vider TOUS les élèves? Irréversible!")) return; setEleves([]); alert("Elèves vidés"); }} className="bg-orange-600 text-white px-3 py-2 rounded">Vider tous les élèves</button>
              <button onClick={()=>{ if(!confirm("Vider toutes les demandes?")) return; setDemandes([]); }} className="bg-orange-600 text-white px-3 py-2 rounded">Vider demandes</button>
              <button onClick={()=>{ if(!confirm("Supprimer tous messages parents & internes?")) return; setMessagesParents([]); setMessagesInternes([]); }} className="bg-[#1B2A4A] text-white px-3 py-2 rounded">Vider tous messages</button>
              <button onClick={()=>{ if(!confirm("Vider annonces?")) return; setAnnonces([]); }} className="bg-[#1B2A4A] text-white px-3 py-2 rounded">Vider annonces</button>
            </div>
            <button onClick={()=>{ const code=prompt("Pour RESET TOTAL, tape RESET-TOTAL"); if(code!=="RESET-TOTAL") return; if(!confirm("DERNIERE CONFIRMATION: TOUT EFFACER?")) return; setEleves([]); setPersonnel([]); setAnnonces([]); setDemandes([]); setMessagesInternes([]); setMessagesParents([]); setFraisTypes([]); setClasses(["CI"]); setMatieresParClasse({}); setAnneesArchivees([]); alert("PLATEFORME REMISE A ZERO - Seuls codes gardés"); }} className="w-full bg-red-700 text-white px-4 py-3 rounded font-bold mt-2">🔥🔥 RESET TOTAL PLATEFORME (Tout effacer sauf codes)</button>
          </div>
        </Panel>
      )}
    </Coquille>
...
async function chargerToutesLesDonnees() {
  ...
  const codes = { directeur: "1234", enseignant: "5678", secretaire: "9012", fondateur: "2026FONDE", enseignants: [{ nom: "Enseignant CI", classe: "CI", code: "ENS-CI-1" }, { nom: "Enseignant CP", classe: "CP", code: "ENS-CP-1" }, { nom: "Enseignant CE1", classe: "CE1", code: "ENS-CE1-1" }, { nom: "Enseignant CE2", classe: "CE2", code: "ENS-CE2-1" }, { nom: "Enseignant CM1", classe: "CM1", code: "ENS-CM1-1" }, { nom: "Enseignant CM2", classe: "CM2", code: "ENS-CM2-1" }, { nom: "Enseignant Maternelle A", classe: "Petite Section", code: "ENS-PS-1" }, { nom: "Enseignant Maternelle B", classe: "Moyenne Section", code: "ENS-MS-1" }] };
  (codesRows || []).forEach((c) => { if(c.role==="enseignants"){ try{ codes.enseignants = JSON.parse(c.code); }catch(e){} } else { codes[c.role] = c.code; } });
...
async function syncCodes(codes) {
  const lignes = Object.entries(codes || {}).map(([role, code]) => {
    if(role==="enseignants"){ return { role, code: JSON.stringify(code||[]) }; }
    return { role, code: typeof code==="string" ? code : String(code||"") };
  });
...
export default function EcoleConnecteeCSP() {
  ...
  const [classeEnseignant, setClasseEnseignant] = useState(null); // enfant identifié par code, pour l'espace parent
  ...
  const entrer = (roleChoisi, extra) => {
    ...
    if (roleChoisi === "enseignant") setClasseEnseignant(extra);
  };
  ...
  const sortir = () => { setRole(null); setEleveIdActif(null); setClasseEnseignant(null); };
  ...
  const Ecran = {
    ...
    enseignant: <EspaceEnseignant eleves={eleves} setEleves={setEleves} classeInitiale={classeEnseignant} />,
    secretaire: (
      <EspaceSecretaire
        eleves={eleves}
        setEleves={setEleves}
    ...
    fondateur: <EspaceFondateur eleves={eleves} personnel={personnel} setPersonnel={setPersonnel} annonces={annonces} setAnnonces={setAnnonces} demandes={demandes} setDemandes={setDemandes} setEleves={setEleves} />,
  }[role];
