...
function EspaceEnseignant({ eleves, setEleves, annonces=[], classeAssignee, onQuitter }) {
  ...
  const mesElevesTous = eleves
    ...
    .map((e) => ({ ...e, absences: e.absences || [], notes: e.notes || [], messages: e.messages || [], frais: e.frais || [] }));
  ...
  const supprimerAbsence = (eleveId, index) => {
    setEleves((prev) => prev.map((el) => {
      ...
      // Supprimer toutes les lignes identiques (doublons inclus)
      const next = liste.filter((a, i) => i !== index && cleAbsObj(a) !== cle);
      return { ...el, absences: dedupeAbsences(next) };
    }));
    try { setTimeout(() => { if (window.__cspFlushEleves) window.__cspFlushEleves(); }, 400); } catch (e) {}
  };
      ...
      {onglet === "absences" && (
        <Panel title="Absences et retards" icon={CalendarClock}>
          <div className="space-y-2">
            ...
            {dedupeAbsences(enfant.absences || []).map((a, i) => (
              <div key={i} className="rounded-lg border px-4 py-2.5 text-[15px]" style={{ borderColor: "#E7DEC8" }}>
                <div className="flex items-center justify-between">
                  <Badge tone={a.type === "Sans motif" ? "or" : "vert"}>{a.type}</Badge>
                  <span className="text-[13px]" style={{ color: "#9A8B67" }}>{a.date}</span>
...
async function chargerToutesLesDonnees() {
  ...
  const eleves = (elevesRows || []).map((e) => ({
    ...
    absences: dedupeAbsences((absencesRows || []).filter((a) => a.eleve_id === e.id).map((a) => ({
...
function fusionnerEleves(locaux, distants) {
  ...
  (locaux || []).forEach((loc) => {
    ...
    const absences = dedupeAbsences([...(d.absences || []), ...(loc.absences || [])]);
...
async function syncEleves(eleves, options = {}) {
  ...
  const absLoc = liste.flatMap((e) => dedupeAbsences(e.absences || []).map((a) => ({
    eleve_id: e.id,
  ...
  // Lignes distantes absentes localement → supprimer en base
  const idsAbsASuppr = (absRemote || [])
    .filter((r) => ids.includes(r.eleve_id) && !localAbs.has(cleAbs(r)))
    .map((r) => r.id)
    .filter(Boolean);
  // Doublons distants (même élève/date/type/motif) → garder une seule ligne
  const vusAbs = new Map();
  (absRemote || []).forEach((r) => {
    if (!ids.includes(r.eleve_id)) return;
    const k = cleAbs(r);
    if (!localAbs.has(k)) return; // déjà dans idsAbsASuppr
    if (vusAbs.has(k)) {
