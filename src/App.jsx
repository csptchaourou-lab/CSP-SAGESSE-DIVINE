...
function EspaceEnseignant({ eleves, setEleves, annonces=[], classeAssignee, onQuitter }) {
  ...
  const ajouterAbsence = (eleveId, nomEleve) => {
    ...
    setEleves((prev) => prev.map((el) => el.id === eleveId
      ? { ...el, absences: [...(el.absences || []), { date: a.date, type: a.type || TYPES_ABSENCE[0], motif: a.motif || "" }] }
      : el));
    setNouvelleAbsence((prev) => ({ ...prev, [eleveId]: { type: TYPES_ABSENCE[0], date: "", motif: "" } }));
    setConfirmation(`${a.type || "Absence"} enregistrée pour ${nomEleve} le ${a.date}`);
  };
...
async function syncEleves(eleves, options = {}) {
  ...
  const idsAbsASuppr = (absRemote || []).filter((r) => ids.includes(r.eleve_id) && !localAbs.has(cleAbs(r))).map((r) => r.id).filter(Boolean);
  if (idsAbsASuppr.length) {
    try { await pgDeleteIn("absences", "id", idsAbsASuppr); } catch (e) { console.warn("suppr abs", e); }
  }
