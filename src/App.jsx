...
async function pgFetch(chemin, options = {}) {
  ...
  const minuteur = setTimeout(() => controle.abort(), 8000);
  try {
...
async function chargerToutesLesDonnees() {
  ...
  const fraisTypes = Array.from(new Map((fraisTypesRows || []).map((f) => [`${String(f.libelle || "").trim().toLowerCase()}::${f.classe || "toutes"}`, { libelle: f.libelle, montant: Number(f.montant), classe: f.classe || null }])).values());
...
async function syncFraisTypes(fraisTypes) {
  ...
  const uniques = Array.from(new Map((fraisTypes || []).map((f) => [`${String(f.libelle || "").trim().toLowerCase()}::${f.classe || "toutes"}`, f])).values());
...
export default function EcoleConnecteeCSP() {
  ...
  const sauvegardeInitiale = useMemo(() => lireStockageLocal(), []);
  const [eleves, setEleves] = useState(sauvegardeInitiale?.eleves || ELEVES_INIT);
  const [personnel, setPersonnel] = useState(sauvegardeInitiale?.personnel || PERSONNEL_INIT);
  const [annonces, setAnnonces] = useState(sauvegardeInitiale?.annonces || ANNONCES_INIT);
  const [demandes, setDemandes] = useState(sauvegardeInitiale?.demandes || DEMANDES_INIT);
  const [ecole, setEcole] = useState(sauvegardeInitiale?.ecole || ECOLE_INIT);
  const [classes, setClasses] = useState(sauvegardeInitiale?.classes || CLASSES_INIT);
  ...
  const [fraisTypes, setFraisTypes] = useState(sauvegardeInitiale?.fraisTypes || FRAIS_TYPES_INIT);
  const [matieresParClasse, setMatieresParClasse] = useState(sauvegardeInitiale?.matieresParClasse || MATIERES_PAR_CLASSE_INIT);
  const [emploiDuTemps, setEmploiDuTemps] = useState(sauvegardeInitiale?.emploiDuTemps || EMPLOI_INIT);
  ...
  useEffect(() => {
    ...
    const actualiser = async () => {
      try {
        ...
        setClasses(d.classes); setFraisTypes(d.fraisTypes.length ? d.fraisTypes : FRAIS_TYPES_INIT);
        setEleves(d.eleves); setPersonnel(d.personnel.length ? d.personnel : PERSONNEL_INIT);
        setEmploiDuTemps(d.emploiDuTemps || {}); setAnneesArchivees(d.anneesArchivees);
        setAnnonces(d.annonces);
        setMessagesInternes(d.messagesInternes); setMessagesParents(d.messagesParents);
        setDemandes(d.demandes); setMatieresParClasse(d.matieresParClasse);
      } catch (err) {
  ...
  useEffect(() => { if (pretPourSync.current && SUPABASE_CONFIGURE) avecSuivi(syncEmploiDuTemps(emploiDuTemps)); }, [emploiDuTemps]);
