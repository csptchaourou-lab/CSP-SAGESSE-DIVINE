...
function fusionnerEleves(locaux, distants) {
  ...
  (locaux || []).forEach((loc) => {
    ...
    // Distant puis local : en cas d'égalité, le local (correction secrétaire) l'emporte
    [...(d.frais || []), ...(loc.frais || [])].forEach((f) => {
      ...
      // IMPORTANT: ne plus faire Math.max(paye) — une correction à la baisse doit rester
      if (tNew > tPrev) {
        fraisMap.set(k, { ...prev, ...f, du, paye: Number(f.paye) || 0 });
      } else if (tPrev > tNew) {
        fraisMap.set(k, { ...f, ...prev, du, paye: Number(prev.paye) || 0 });
      } else {
        // Même horodatage ou absent : la dernière valeur parcourue gagne (local en dernier)
        fraisMap.set(k, {
          ...prev, ...f, du,
  ...
  for (const f of fraisAMaj) {
    try {
      await pgFetch(`/frais_eleve?id=eq.${f.id}`, {
        ...
        headers: { Prefer: "return=minimal" },
        body: JSON.stringify({
          du: f.du, paye: f.paye, mode: f.mode,
      ...
      await pgInsert("frais_eleve", [{
        eleve_id: f.eleve_id, libelle: f.libelle, du: f.du, paye: f.paye, mode: f.mode,
        date_paiement: f.date_paiement || f.datePaiement || null,
      }]);
    }
  }
    ...
    const tenterChargement = () => {
      ...
      chargerToutesLesDonnees()
        ...
        .catch((err) => {
          ...
          // Réessayer quelques fois, puis ouvrir l'app quand même (données locales)
          if (essais < 5) {
            setTimeout(tenterChargement, Math.min(2000 * essais, 8000));
          } else {
            setBasePrete(true);
            setChargement(false);
            pretPourSync.current = true;
            setErreurChargement(
              (err.message || String(err)) +
              " — Base temporairement injoignable. Mode local activé. Vérifiez VITE_SUPABASE_URL et VITE_SUPABASE_ANON_KEY sur Vercel, puis actualisez."
            );
          }
        });
    };
      ...
      <div className="flex min-h-screen w-full items-center justify-center px-6" style={{ background: "#FAF6EE" }}>
        <div className="max-w-md text-center">
          ...
          <p className="mt-2 text-[14px]" style={{ color: "#5C5240" }}>
            Chargement des données de l&apos;école (élèves, frais, codes). L&apos;écran d&apos;accueil s&apos;affiche dès que la base répond.
          </p>
          {erreurChargement && (
            <p className="mt-3 text-[13px] font-semibold" style={{ color: "#8A2E2E" }}>
              {erreurChargement}
            </p>
          )}
          <button
            type="button"
            className="mt-5 rounded-lg px-4 py-2.5 text-[14px] font-bold"
            style={{ background: "#1B2A4A", color: "#C9A227" }}
            onClick={() => {
              setBasePrete(true);
              setChargement(false);
