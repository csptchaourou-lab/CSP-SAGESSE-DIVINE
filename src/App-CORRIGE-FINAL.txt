...
async function pgFetch(chemin, options = {}) {
  ...
  // 45s: les sync (notes/frais) font plusieurs requêtes et peuvent dépasser 8s sur mobile
  const minuteur = setTimeout(() => controle.abort(), 45000);
  try {
...
async function syncMessagesInternes(messagesInternes) {
  if (!messagesInternes || messagesInternes.length === 0) return;
  const lignes = messagesInternes.map((m) => ({
    id: `${m.destinataire || 'x'}_${m.date || Date.now()}_${(m.texte||'').slice(0,10)}`.slice(0,80),
    destinataire: m.destinataire ?? null, texte: m.texte ?? null, date: m.date ?? null,
  }));
  await pgUpsert("messages_internes", lignes, "id");
}

async function syncMessagesParents(messagesParents) {
  // CORRECTION: on n'efface plus tout (sinon 1 téléphone vide efface l'autre)
  // On insère seulement ce qui n'existe pas encore
  if (!messagesParents || messagesParents.length === 0) return;
  // On prend les 20 derniers messages et on les upsert avec un id unique date+eleve
  const lignes = messagesParents.map((m) => ({
    id: `${m.eleveId || 'x'}_${m.date || Date.now()}_${(m.texte||'').slice(0,10)}`.slice(0,80),
    eleve_id: m.eleveId ?? null, nom_eleve: m.nomEleve ?? null, classe: m.classe ?? null,
    texte: m.texte ?? null, date: m.date ?? null, lu: m.lu ?? false,
  }));
  await pgUpsert("messages_parents", lignes, "id");
}
...
async function syncEleves(eleves) {
  ...
  // CORRECTION: on ne supprime plus toutes les notes/absences à chaque sync
  // On upsert seulement - ça évite que les notes de l'enseignant disparaissent chez les parents
  // On garde juste la suppression des élèves vraiment supprimés (déjà fait plus haut)

  await pgUpsert("notes", liste.flatMap((e) => (e.notes || []).map((n) => ({
    id: `${e.id}_${n.matiere||""}_${n.evaluation||""}_${n.mois||""}`.slice(0,80), eleve_id: e.id,
  ...
  await pgUpsert("absences", liste.flatMap((e) => (e.absences || []).map((a) => ({
    id: `${e.id}_${a.date||""}_${a.type||""}`.slice(0,80),
    eleve_id: e.id, date: a.date ?? null, type: a.type ?? null, motif: a.motif ?? null,
  }))));

  await pgUpsert("frais_eleve", liste.flatMap((e) => (e.frais || []).map((f) => ({
    id: `${e.id}_${f.libelle||""}_${f.datePaiement||""}`.slice(0,80),
    eleve_id: e.id, libelle: f.libelle ?? null, du: f.du ?? 0, paye: f.paye ?? 0,
    mode: f.mode ?? null, date_paiement: f.datePaiement ?? null,
  }))));

  await pgUpsert("messages_eleve", liste.flatMap((e) => (e.messages || []).map((m) => ({
    id: `${e.id}_${m.date||""}_${(m.texte||"").slice(0,10)}`.slice(0,80),
...
export default function EcoleConnecteeCSP() {
  ...
  // --- SYNCHRO TEMPS REEL: recharge toutes les 10s pour que parent voit notes et directeur voit messages ---
  useEffect(() => {
    if (!SUPABASE_CONFIGURE) return;
    const interval = setInterval(async () => {
      if (sourceRemoteRef.current) return;
      try {
        const d = await chargerToutesLesDonnees();
        // On met à jour seulement si différent
        if (JSON.stringify(d.eleves) !== JSON.stringify(eleves)) {
          sourceRemoteRef.current = true;
          setEleves(d.eleves);
          setTimeout(() => { sourceRemoteRef.current = false; }, 800);
        }
        if (JSON.stringify(d.messagesParents) !== JSON.stringify(messagesParents)) {
          sourceRemoteRef.current = true;
          setMessagesParents(d.messagesParents);
          setTimeout(() => { sourceRemoteRef.current = false; }, 800);
        }
