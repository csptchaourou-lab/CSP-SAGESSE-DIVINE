...
function Connexion({ onEntrer, onInscription, eleves }) {
  ...
  const valider = () => {
    ...
    if (choix === "enseignant") {
      // nouveau système : 1 code par classe
      ...
      // compat ancien code global 5678
      if (saisie === codes.enseignant || saisie === codes["enseignant"]) { setErreur(""); onEntrer("enseignant", Object.keys(map)[0] || "6ème A"); return; }
      ...
      <div className="mx-auto flex min-h-screen max-w-5xl flex-col items-center px-6 py-12 sm:justify-center sm:py-16">
        ...
        {choix && (
          <div ref={zoneCodeRef} className="mt-6 w-full max-w-md scroll-mt-8">
            ...
            {choix !== "parent" && choix !== "enseignant" && !erreur && (
            ...
            {choix === "enseignant" && !erreur && (
              <div className="mt-1.5 px-1 text-[13px]" style={{ color: "#9A8B67" }}><div>Codes de démo (1 par classe) :</div><div className="mt-1 flex flex-wrap gap-1">{Object.entries(codes.enseignants||{}).slice(0,4).map(([cls,cd])=> (<span key={cls} className="rounded bg-white px-1.5 py-0.5 font-mono text-[11px]" style={{border:"1px solid #E7DEC8"}}>{cls}: {cd}</span>))}</div></div>
            )}
          </div>
        )}
...
function EspaceFondateur({ demandes=[], setDemandes, eleves=[], personnel=[], annonces=[], setAnnonces, setEleves, ecole={}, setEcole, classes=[], setClasses, fraisTypes=[], setFraisTypes, anneeScolaire="", setAnneeScolaire, codes={}, setCodes, matieresParClasse={}, setMatieresParClasse }) {
  // Fondateur = Directeur avec droits complets + gestion codes
  return (
    <EspaceDirecteur
      eleves={eleves}
      personnel={personnel}
...
const supprimerDemande = (demande) => {
  ...
  const [deplacementSource, setDeplacementSource] = useState("");
  const [deplacementDest, setDeplacementDest] = useState("");
  const deplacerTousEleves = (source, dest) => {
    if(!source || !dest || source===dest) return;
    if(!confirm(`Déplacer TOUS les ${eleves.filter(e=>e.classe===source).length} élèves de ${source} vers ${dest}?`)) return;
    setEleves(prev => prev.map(e => e.classe===source ? {...e, classe: dest} : e));
    // aussi deplacer matieres? non
  };
    ...
    <Coquille role="directeur" roleLabel="Espace Administratif" nav={nav} actif={onglet} setActif={setOnglet}
      ...
      {onglet === "vueClasse" && (
        <>
          ...
          {/* ACTION DIRECTEUR : RÉAFFECTER TOUS LES ÉLÈVES */}
          <div className="mb-4 flex flex-wrap items-center gap-2 rounded-lg border p-3" style={{borderColor:"#B8D4FF", background:"#EEF6FF"}}>
            <span className="text-[13px] font-bold" style={{color:"#1B2A4A"}}>🔄 Option groupe : Déplacer tous les élèves de</span>
            <select value={deplacementSource} onChange={e=>setDeplacementSource(e.target.value)} className="rounded-full border px-3 py-1 text-[13px]" style={{borderColor:"#B8D4FF"}}>
              <option value="">Source...</option>
              {classes.map(c=><option key={c} value={c}>{c}</option>)}
            </select>
            <span className="text-[13px]">vers</span>
            <select value={deplacementDest} onChange={e=>setDeplacementDest(e.target.value)} className="rounded-full border px-3 py-1 text-[13px]" style={{borderColor:"#B8D4FF"}}>
              <option value="">Destination...</option>
              {classes.map(c=><option key={c} value={c}>{c}</option>)}
            </select>
            <button onClick={()=>{deplacerTousEleves(deplacementSource, deplacementDest); setDeplacementSource(""); setDeplacementDest("");}} className="rounded-full px-3 py-1.5 text-[13px] text-white" style={{background:"#1B2A4A"}}>Déplacer tous</button>
            <span className="text-[12px]" style={{color:"#5C5240"}}>{deplacementSource? `${eleves.filter(e=>e.classe===deplacementSource).length} élèves concernés` : ''}</span>
          </div>
  ...
  <div>
    <div className="font-bold" style={{color:"#1B2A4A"}}>{e.prenom} {e.nom} <span className="font-mono text-[12px]" style={{color:"#9A8B67"}}>{e.id}</span></div>
    <div className="text-[13px]" style={{color:"#5C5240"}}>Notes: {e.notes.length} | Absences: {e.absences.length} | Paye: {e.frais.reduce((a,f)=>a+f.paye,0)}F | Classe: {e.classe}</div>
  </div>
  <div className="flex items-center gap-2">
    <select value={e.classe} onChange={(ev)=>{
      const nouvelleClasse = ev.target.value;
      if(!nouvelleClasse || nouvelleClasse===e.classe) return;
      if(!confirm(`Déplacer ${e.prenom} ${e.nom} de ${e.classe} vers ${nouvelleClasse}?`)) return;
      setEleves(prev=> prev.map(el=> el.id===e.id ? {...el, classe: nouvelleClasse} : el));
    }} className="rounded-full border px-3 py-1.5 text-[13px]" style={{borderColor:"#B8D4FF"}}>
      {classes.map(c=><option key={c} value={c}>{c}</option>)}
    </select>
    <span className="text-[11px] px-2 py-1 rounded-full" style={{background:"#EEF6FF", color:"#1B2A4A"}}>↔️ Changer</span>
  </div>
</div>)}
              </div>
            );
        ...
        <>
          ...
          <Panel title="Codes d'accès du personnel" icon={KeyRound}>
            ...
            <p className="mt-3 text-[13px]" style={{ color: "#9A8B67" }}>
              Code Parent = code élève CSPxxxx. Code Enseignant = code de sa classe uniquement.
            </p>
          </Panel>
        </>
      )}
...
function EspaceEnseignant({ eleves, setEleves, classeAssignee }) {
  ...
  useEffect(()=>{ if(classeAssignee) setMaClasse(classeAssignee); }, [classeAssignee]);
  ...
  return (
    <Coquille role="enseignant" roleLabel="Mme Houssou · Enseignante" nav={nav} actif={onglet} setActif={setOnglet}
      ...
      {annonces && annonces.length>0 && (
        <div className=\"mb-4 rounded-lg border p-4\" style={{borderColor:\"#E7DEC8\", background:\"#FFF9E6\"}}>
          <h3 className=\"font-semibold flex items-center gap-2\" style={{color:\"#1B2A4A\"}}><Megaphone size={16}/> Messages du Directeur (visibles par tous)</h3>
          <div className=\"mt-2 space-y-2\">
            {annonces.map((a,i)=>(<div key={i} className=\"text-[14px] p-2 bg-white rounded border\" style={{borderColor:\"#E7DEC8\"}}>{a.contenu || a.texte || JSON.stringify(a)}</div>))}
          </div>
        </div>
      )}
...
async function chargerToutesLesDonnees() {
  ...
  const classesInitiales = ["6ème A","6ème B","5ème A","5ème B","4ème A","4ème B","3ème A","3ème B","Seconde A","Première A","Terminale A"];
  const codesEnseignantsInitiaux = {};
  classesInitiales.forEach((c,i)=>{ codesEnseignantsInitiaux[c] = `ENS-${c.replace(/[^A-Z0-9]/gi,"").toUpperCase()}-${1000+i*111}`; });
  const codes = { directeur: "1234", enseignant: "5678", secretaire: "9012", fondateur: "0000", enseignants: codesEnseignantsInitiaux };
...
!navigator.onLine){ console.warn('Sync offline', err); return; }
  ...
  const Ecran = {
    ...
    enseignant: <EspaceEnseignant eleves={eleves} setEleves={setEleves} annonces={annonces} classeAssignee={classeEnseignantActive} />,
    secretaire: (
      <EspaceSecretaire
        eleves={eleves}
        setEleves={setEleves}
