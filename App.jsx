...
const supprimerDemande = (demande) => {
  ...
  <div className="flex items-center gap-2">
    ...
    <span className="text-[11px] px-2 py-1 rounded-full" style={{background:"#EEF6FF"}}>↔️ Changer classe</span>
      ...
      {onglet === "annonces" && (
        <Panel title="Annonces de l'école" icon={Megaphone}>
          <div className="space-y-3">
            {(!annonces || annonces.length === 0) && <p className="text-[15px] font-bold" style={{ color: "#9A8B67" }}>Aucune annonce pour le moment.</p>}
            {annonces && annonces.map((a, i) => (
              <div key={i} className="rounded-xl border px-4 py-3" style={{ borderColor: "#E7DEC8", background: "#FAF6EE" }}>
                <div className="flex items-center justify-between">
                  <span className="font-bold text-[16px]" style={{ color: "#1B2A4A" }}>{a.titre}</span>
                  <span className="text-[12px] font-bold" style={{ color: "#9A8B67" }}>{a.date}</span>
                </div>
                <p className="mt-2 text-[15px] font-semibold" style={{ color: "#3E3625" }}>{a.texte}</p>
              </div>
            ))}
          </div>
        </Panel>
