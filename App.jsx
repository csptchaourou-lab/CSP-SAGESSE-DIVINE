...
import {
  ...
  LogOut, ShieldCheck, KeyRound, Bell, CircleUserRound, Award,
...
const CODES_INIT = { fondateur: "0007", directeur: "1234", enseignant: "5678", secretaire: "9012" };
...
const ROLES = [
  { id: "fondateur", label: "Espace Fondateur", icon: Award, desc: "Supervision générale de l'établissement" },
...
function EspaceDirecteur({ eleves, personnel, setPersonnel, annonces, setAnnonces, demandes, setDemandes, setEleves, isFondateur }) {
  ...
  const changerClasseEleve = (eleveId, nouvelleClasseNom) => {
    setEleves((prev) => prev.map((e) => e.id === eleveId ? { ...e, classe: nouvelleClasseNom } : e));
  };
  ...
  const nav = [
    ...(isFondateur ? [{ id: "fondateur", label: "Vue Fondateur", icon: Award }] : []),
  ...
  return (
    <Coquille role="directeur" roleLabel={isFondateur ? "Espace Fondateur" : "Espace Administratif"} nav={nav} actif={onglet} setActif={setOnglet}
                  ...
                  {inscrits.length > 0 && (
                    <details className="mt-1.5">
                      ...
                      <div className="mt-1.5 space-y-1.5">
                        {inscrits.map((e) => (
                          <div key={e.id} className="flex flex-wrap items-center justify-between gap-1.5 rounded border px-2 py-1.5 text-[13px]" style={{ borderColor: "#E7DEC8" }}>
                            <div>
                              <span style={{ color: "#5C5240" }}>{e.prenom} {e.nom}</span>
                              <span className="ml-1.5 font-mono" style={{ color: "#1B2A4A" }}>{e.id}</span>
                            </div>
                            <select
                              value={e.classe}
                              onChange={(ev) => changerClasseEleve(e.id, ev.target.value)}
          ...
          <Panel title="Codes d'accès du personnel" icon={KeyRound}>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              {isFondateur ? (
                ...
                <div>
                  <label className="mb-1 block text-[13px] font-medium" style={{ color: "#5C5240" }}>Code Directeur</label>
                  <div className="rounded-lg border px-3 py-2 text-[15px]" style={{ borderColor: "#E7DEC8", color: "#9A8B67", background: "#F1ECDD" }}>
                    •••• (modifiable par le Fondateur uniquement)
                  </div>
                </div>
              )}
      ...
      {onglet === "fondateur" && isFondateur && (
        <Panel title="Vue d'ensemble Fondateur" icon={Award}>
          <p className="mb-4 text-[13px]" style={{ color: "#9A8B67" }}>
            Supervision générale, toutes années confondues — accessible uniquement depuis l'Espace Fondateur.
          </p>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <Stat label="Élèves inscrits (actuels)" value={eleves.length} tone="ardoise" />
            <Stat label="Années archivées" value={anneesArchivees.length} tone="ardoise" />
            <Stat
              label="Élèves — toutes années"
              value={eleves.length + anneesArchivees.reduce((s, a) => s + (a.eleves?.length || 0), 0)}
              tone="or"
            />
            <Stat label="Membres du personnel" value={personnel.length} tone="vert" />
          </div>
          <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
            <Stat
              label="Frais encaissés — année en cours"
              value={`${eleves.reduce((s, e) => s + e.frais.reduce((a, f) => a + f.paye, 0), 0).toLocaleString("fr-FR")} F`}
              tone="or"
            />
            <Stat
              label="Frais encaissés — historique archivé"
              value={`${anneesArchivees.reduce((s, a) => s + (a.eleves || []).reduce((s2, e) => s2 + (e.frais || []).reduce((a2, f) => a2 + (f.paye || 0), 0), 0), 0).toLocaleString("fr-FR")} F`}
              tone="vert"
            />
          </div>
          {anneesArchivees.length > 0 && (
            <div className="mt-5">
              <h4 className="mb-2 text-[12px] font-medium uppercase tracking-wide" style={{ color: "#9A8B67" }}>Historique par année</h4>
              <div className="space-y-2">
                {anneesArchivees.map((a, i) => (
                  <div key={i} className="flex items-center justify-between rounded-lg border px-4 py-2.5 text-[15px]" style={{ borderColor: "#E7DEC8" }}>
                    <span style={{ color: "#1B2A4A" }} className="font-medium">{a.annee}</span>
                    <Badge tone="ardoise">{(a.eleves || []).length} élève{(a.eleves || []).length > 1 ? "s" : ""}</Badge>
                  </div>
                ))}
              </div>
            </div>
          )}
          <p className="mt-4 text-[13px]" style={{ color: "#9A8B67" }}>
            Le Directeur assure la gestion opérationnelle quotidienne ; le Fondateur garde ici un droit de regard permanent, et reste le seul à pouvoir modifier le code d'accès du Directeur.
          </p>
        </Panel>
      )}
    </Coquille>
...
function EspaceEnseignant({ eleves, setEleves, annonces, messagesInternes }) {
  const { matieresParClasse, setMatieresParClasse } = useEcole();
  ...
  const envoyerAppreciation = (eleveId, nomEleve) => {
    ...
    if (!texte) return;
    setEleves((prev) => prev.map((el) => el.id === eleveId
      ? { ...el, messages: [...el.messages, { auteur: "Enseignant(e)", texte, date: "aujourd'hui" }] }
      : el));
    setAppreciationSaisie((prev) => ({ ...prev, [eleveId]: "" }));
    setConfirmation(`Appréciation envoyée au parent de ${nomEleve}`);
  };
  ...
  const nav = [
    ...
    { id: "matieres", label: "Matières", icon: FileText },
    { id: "info", label: "Annonces & messages", icon: Megaphone },
  ];
  ...
  return (
    <Coquille role="enseignant" roleLabel="Mme Houssou · Enseignante" nav={nav} actif={onglet} setActif={setOnglet}
      ...
      {onglet === "info" && (
        <>
          <Panel title="Annonces de l'établissement" icon={Megaphone}>
            <div className="space-y-2">
          ...
          <Panel title="Messages de la direction" icon={MessageSquare}>
            <div className="space-y-2">
              {(messagesInternes || []).filter((m) => m.destinataire === "tous" || m.destinataire === "Mme Houssou").length === 0 && (
                <p className="text-[15px]" style={{ color: "#9A8B67" }}>Aucun message reçu pour le moment.</p>
              )}
              {(messagesInternes || [])
                .filter((m) => m.destinataire === "tous" || m.destinataire === "Mme Houssou")
                .map((m, i) => (
                  <div key={i} className="rounded-lg border px-4 py-2.5" style={{ borderColor: "#E7DEC8" }}>
                    <div className="flex items-center justify-between">
                      <Badge tone={m.destinataire === "tous" ? "or" : "ardoise"}>
                        {m.destinataire === "tous" ? "Tout le personnel" : "Message personnel"}
                      </Badge>
                      <span className="text-[13px]" style={{ color: "#9A8B67" }}>{m.date}</span>
                    </div>
                    <p className="mt-1 text-[15px]" style={{ color: "#5C5240" }}>{m.texte}</p>
                  </div>
                ))}
            </div>
          </Panel>
...
function EspaceSecretaire({ eleves, setEleves, demandes, setDemandes, annonces, setAnnonces, messagesInternes }) {
  ...
  const enregistrerPaiement = (eleve) => {
    ...
    setEleves((prev) => prev.map((e) => e.id === eleve.id
      ? {
          ...e,
          frais: e.frais.map((f) => {
            const ajout = parseFloat(montants[f.libelle]) || 0;
            if (ajout <= 0) return f;
            return { ...f, paye: f.paye + ajout, mode, datePaiement: "aujourd'hui" };
          }),
        }
      : e));
    setConfirmationPaiement(`${total.toLocaleString("fr-FR")} F enregistrés pour ${eleve.prenom} ${eleve.nom} (${mode}).`);
  ...
  const nav = [
    ...
    { id: "frais", label: "Frais scolaires", icon: Wallet },
    { id: "annonces", label: "Annonces", icon: Megaphone },
    { id: "messages", label: "Messages reçus", icon: MessageSquare },
  ];
  ...
  return (
    <Coquille role="secretaire" roleLabel="Mlle Dossou · Secrétaire" nav={nav} actif={onglet} setActif={setOnglet}
      ...
      {onglet === "messages" && (
        <Panel title="Messages de la direction" icon={MessageSquare}>
          <div className="space-y-2">
            {(messagesInternes || []).filter((m) => m.destinataire === "tous" || m.destinataire === "Mlle Dossou").length === 0 && (
              <p className="text-[15px]" style={{ color: "#9A8B67" }}>Aucun message reçu pour le moment.</p>
            )}
            {(messagesInternes || [])
              .filter((m) => m.destinataire === "tous" || m.destinataire === "Mlle Dossou")
              .map((m, i) => (
                <div key={i} className="rounded-lg border px-4 py-2.5" style={{ borderColor: "#E7DEC8" }}>
                  <div className="flex items-center justify-between">
                    <Badge tone={m.destinataire === "tous" ? "or" : "ardoise"}>
...
function EspaceParent({ eleves, setEleves, eleveId, personnel, annonces }) {
  const { setMessagesParents, matieresParClasse } = useEcole();
  ...
  // Moyenne pondérée par le coefficient de chaque matière (comme sur un vrai bulletin) —
  // si une matière n'a pas de coefficient défini pour la classe, on prend 1 par défaut.
  const coefMatiere = (nomMatiere) => {
    const liste = matieresParClasse?.[enfant.classe] || [];
    const trouvee = liste.find((m) => m.nom === nomMatiere);
    return trouvee ? trouvee.coef : 1;
  };

  const moyenne = useMemo(() => {
    if (!enfant.notes.length) return null;
    const totalPoints = enfant.notes.reduce((s, n) => s + n.note20 * coefMatiere(n.matiere), 0);
    const totalCoefs = enfant.notes.reduce((s, n) => s + coefMatiere(n.matiere), 0);
    if (totalCoefs === 0) return null;
    return Math.round((totalPoints / totalCoefs) * 10) / 10;
  }, [enfant, matieresParClasse]);
...
export default function EcoleConnecteeCSP() {
  ...
  const Ecran = {
    ...
    enseignant: <EspaceEnseignant eleves={eleves} setEleves={setEleves} annonces={annonces} messagesInternes={messagesInternes} />,
    secretaire: (
      <EspaceSecretaire
        eleves={eleves}
        setEleves={setEleves}
        ...
        annonces={annonces}
        setAnnonces={setAnnonces}
        messagesInternes={messagesInternes}
      />
    ),
    parent: <EspaceParent eleves={eleves} setEleves={setEleves} eleveId={eleveIdActif} personnel={personnel} annonces={annonces} />,
  }[role];
