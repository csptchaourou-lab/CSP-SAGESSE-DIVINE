...
function Connexion({ onEntrer, onInscription, eleves }) {
  const [choix, setChoix] = useState(null); const [code, setCode] = useState(""); const [erreur, setErreur] = useState(""); const { ecole, codes } = useEcole(); const zoneCodeRef = useRef(null);
  ...
  const valider = () => {
    ...
    if (saisie !== codes[choix]) { setErreur("Code incorrect."); return; } setErreur(""); onEntrer(choix);
  };
  return (
...
function EspaceEnseignant({ eleves, setEleves, classeInitiale }) {
  ...
  const matieres = matieresParClasse[maClasse] || matieresParClasse[maClasse.toUpperCase()] || matieresParClasse["CM1"] || [];
...
function EspaceDirecteur({ eleves, personnel, setPersonnel, annonces, setAnnonces, demandes, setDemandes, setEleves }) {
  const { ecole, setEcole, classes, setClasses, fraisTypes, setFraisTypes, anneeScolaire, setAnneeScolaire, codes, setCodes, codesClasses, setCodesClasses, matieresParClasse, setMatieresParClasse, messagesInternes, setMessagesInternes, anneesArchivees, setAnneesArchivees, messagesParents, setMessagesParents } = useEcole();
  ...
  const [anneeArchiveeConf, setAnneeArchiveeConf] = useState(false);
  ...
  const archiverAnneeActuelle = () => {
    if(!confirm(`Archiver l'année ${anneeScolaire} ?`)) return;
    const archive = { annee: anneeScolaire, dateArchivage: formatDateFr(), eleves: eleves, effectif: eleves.length, classes: classes, fraisTypes: fraisTypes, donnees: eleves };
    setAnneesArchivees(prev=>[archive, ...prev]);
    setAnneeArchiveeConf(true);
    setTimeout(()=>setAnneeArchiveeConf(false), 3000);
  };
  const demarrerNouvelleAnnee = () => {
    const nouvelleAnnee = prompt("Nouvelle année scolaire (ex: 2027-2028) :", "");
    if(!nouvelleAnnee || !nouvelleAnnee.trim()) return;
    if(!confirm(`Démarrer ${nouvelleAnnee.trim()} va vider les élèves. Continuer ?`)) return;
    setAnneeScolaire(nouvelleAnnee.trim());
    setEleves([]);
    setDemandes([]);
    setAnnonces([{ titre: `Bonne rentrée ${nouvelleAnnee.trim()}`, texte: `Début de la nouvelle année ${nouvelleAnnee.trim()}`, date: formatDateFr() }]);
    setMessagesInternes([]);
    setMessagesParents([]);
  };
  const restaurerArchive = (archive) => {
    if(!confirm(`Restaurer l'année ${archive.annee} ?`)) return;
    setEleves(archive.eleves || archive.donnees || []);
    setAnneeScolaire(archive.annee);
  };
  const mettreAJourFraisEtEleves = (index, champ, valeur) => {
    const nouveauMontant = champ==="montant" ? (Number(valeur)||0) : valeur;
    const ancienLibelle = fraisTypes[index]?.libelle;
    setFraisTypes(prev=>prev.map((x,i)=> i===index ? {...x, [champ]: nouveauMontant} : x));
    if(champ==="libelle" || champ==="montant"){
      setEleves(prev=>prev.map(e=>{
        const fraisMaj = e.frais.map(f=>{
          if(f.libelle===ancienLibelle){
            return {...f, libelle: champ==="libelle"? valeur : f.libelle, du: champ==="montant"? nouveauMontant : f.du };
          }
          return f;
        });
        return {...e, frais: fraisMaj};
      }));
    }
  };
  ...
  const nav = [
    ...
    { id: "classes", label: "Classes + Élèves", icon: School },
    { id: "matieres", label: "Matières par classe", icon: FileText },
    { id: "frais", label: "Frais (fixe prix)", icon: Wallet },
    { id: "personnel", label: "Personnel", icon: Users },
    { id: "archivage", label: "Archivage Années", icon: FileText },
    { id: "parametres", label: "Paramètres + Codes par classe", icon: Settings }
  ];

  const elevesDeClasse = (classeNom) => eleves.filter(e=>(e.classe||"").toUpperCase()===classeNom.toUpperCase());
  ...
  return (
    <Coquille role="directeur" roleLabel="Espace Administratif" nav={nav} actif={onglet} setActif={setOnglet} onQuitter={() => {}}>
      ...
      {onglet === "inscriptions" && (
        <Panel title="Inscriptions en ligne - Validation Directeur UNIQUEMENT (Claude avait créé)" icon={Inbox}>
          <p className="mb-4 text-[13px]" style={{color:"#9A8B67"}}>Claude avait créé inscriptions. Nous avons amélioré: seul directeur valide, code CSP auto-généré.</p>
              ...
              <div key={i} className="rounded-xl border p-4" style={{borderColor:"#E7DEC8", background: d.statut==="en_attente"?"#FFFDF6":"white"}}>
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="text-[13px]" style={{color:"#9A8B67"}}>Parrain: {d.parrainNom} ({d.parrainTelephone}) - {d.date}</div>
                  </div>
      ...
      {onglet === "messages_parents" && (
        <Panel title="Messages des Parrains / Parents → Directeur SEULEMENT (NOUVEAU - nous avons ajouté)" icon={MessageSquare}>
          <p className="mb-3 text-[13px]" style={{color:"#9A8B67"}}>NOUVEAU: Ces messages sont envoyés par parents depuis leur espace et visibles QUE par directeur. Claude n'avait pas ça.</p>
          <div className="space-y-3">
      ...
      {onglet === "messages_internes" && (
        <>
          <Panel title="Envoyer message interne → Tout le personnel (NOUVEAU - nous avons ajouté)" icon={MessageSquare}>
            <p className="mb-2 text-[12px]" style={{color:"#9A8B67"}}>NOUVEAU: Directeur envoie à tout personnel / enseignants / secrétaire / personnel précis. Claude n'avait pas ça.</p>
            <div className="space-y-3">
              ...
              <textarea value={msgInterne.texte} onChange={(e)=>setMsgInterne({...msgInterne, texte:e.target.value})} rows={3} placeholder="Message interne..." className="w-full rounded-lg border px-3 py-2 text-[15px] outline-none" style={{borderColor:"#E7DEC8"}} />
            ...
            <div className="space-y-2">
              {messagesInternes.map((m,i)=>(
                <div key={i} className="flex justify-between rounded-lg border px-3 py-2" style={{borderColor:"#E7DEC8"}}>
                  <div><Badge tone="or">{m.destinataire}</Badge><p className="mt-1 text-[14px]">{m.texte}</p></div>
      ...
      {onglet === "annonces_pub" && (
        <>
          <Panel title="Publier annonce publique → Personnel + TOUS les Parents (NOUVEAU - amélioré)" icon={Megaphone}>
            <p className="mb-2 text-[12px]" style={{color:"#9A8B67"}}>Claude avait annonces basiques. Nous avons ajouté: visible personnel + parents, avec titre + contenu.</p>
            <div className="space-y-3">
              <Champ label="Titre" value={annoncePub.titre} onChange={(e)=>setAnnoncePub({...annoncePub, titre:e.target.value})} />
              <div><label className="mb-1 block text-[13px] font-medium" style={{color:"#5C5240"}}>Contenu</label><textarea value={annoncePub.texte} onChange={(e)=>setAnnoncePub({...annoncePub, texte:e.target.value})} rows={3} className="w-full rounded-lg border px-3 py-2 text-[15px] outline-none" style={{borderColor:"#E7DEC8"}} /></div>
              <div className="flex justify-end"><Btn variant="gold" icon={Send} onClick={envoyerAnnoncePublique}>Publier pour tous</Btn></div>
            </div>
          </Panel>
          <Panel title="Annonces publiées" icon={Megaphone}>
      ...
      {onglet === "classes" && (
        <div className="space-y-4">
          <Panel title="Classes + Élèves - Cliquez pour voir détails (AMÉLIORÉ - directeur voit tout)" icon={School}>
            <p className="mb-2 text-[12px]" style={{color:"#9A8B67"}}>Claude: affichage seulement. NOUVEAU: directeur clique sur classe et voit liste complète élèves avec frais, notes, parrain. + Ajouter/Supprimer classe. + Kponjè ajouté (9 classes).</p>
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 mb-4">
              {classes.map((c,i)=>(
                <button key={i} onClick={()=>setClasseSelectionnee(c===classeSelectionnee? null : c)} className="rounded-lg border px-3 py-2 flex justify-between text-left" style={{borderColor: classeSelectionnee===c?"#C9A227":"#E7DEC8", background: classeSelectionnee===c?"#FFFDF6":"white"}}>
                  <span className="font-medium" style={{color:"#1B2A4A"}}>{c}</span>
                  <div className="flex gap-2 items-center"><Badge>{elevesDeClasse(c).length}</Badge><Trash2 size={12} className="cursor-pointer" style={{color:"#8A2E2E"}} onClick={(e)=>{ e.stopPropagation(); if(confirm(`Supprimer ${c}? Attention: tous les élèves de ${c} resteront mais classe supprimée.`)) { setClasses(prev=>prev.filter(x=>x!==c)); setCodesClasses(prev=>{ const copy={...prev}; delete copy[c]; return copy; }); } }} /></div>
                </button>
              ))}
            </div>
            <div className="flex gap-2 rounded-lg p-3" style={{background:"#F1ECDD"}}>
              ...
              <Btn variant="gold" icon={Plus} onClick={()=>{ if(!nouvelleClasse.trim()) return; const nomClasse = nouvelleClasse.trim(); setClasses(prev=>[...prev, nomClasse]); setCodesClasses(prev=>{ if(prev[nomClasse]) return prev; const codeAuto = String(1000 + Object.keys(prev).length + 1); return {...prev, [nomClasse]: codeAuto}; }); setNouvelleClasse(""); }}>Ajouter</Btn>
            </div>
            {classeSelectionnee && (
              <div className="mt-6 rounded-xl border-2 p-4" style={{borderColor:"#C9A227", background:"#FFFDF6"}}>
                <div className="flex justify-between items-center mb-3">
                  <h4 className="font-serif text-lg" style={{color:"#1B2A4A"}}>Classe {classeSelectionnee} - {elevesDeClasse(classeSelectionnee).length} élèves - Directeur voit tout (frais, notes, parent)</h4>
                  <Btn small variant="ghost" onClick={()=>setClasseSelectionnee(null)}>Fermer</Btn>
                </div>
                <div className="space-y-3 max-h-[600px] overflow-y-auto">
                  {elevesDeClasse(classeSelectionnee).length===0 && <p style={{color:"#9A8B67"}}>Aucun élève.</p>}
                  {elevesDeClasse(classeSelectionnee).map((e, idx)=>(
                    <div key={idx} className="rounded-lg border bg-white p-3" style={{borderColor:"#E7DEC8"}}>
                      <div className="flex justify-between">
                        <div className="font-medium text-[14px]" style={{color:"#1B2A4A"}}>{e.prenom} {e.nom} - {e.id}</div>
                        <Badge tone={e.absences.length?"or":"vert"}>{e.absences.length} abs</Badge>
                      </div>
                      <div className="text-[12px] mt-1" style={{color:"#5C5240"}}>Parrain: {e.parent?.nom} - {e.parent?.telephone} | Ens: {e.enseignant}</div>
                      <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
                        <div className="rounded p-2" style={{background:"#F1ECDD"}}>
                          <div className="text-[12px] font-medium">FRAIS (secrétaire → parent + directeur en même temps)</div>
                          {(e.frais||[]).map((f,i)=>(
                            <div key={i} className="flex justify-between text-[12px]"><span>{f.libelle}: {f.paye}/{f.du} F {f.paye>=f.du?"✓":""}</span><span style={{color:"#9A8B67"}}>{f.mode||""} {f.datePaiement||""}</span></div>
                          ))}
                          <div className="text-[11px] mt-1">Reste: {(e.frais||[]).reduce((s,f)=>s+f.du,0) - (e.frais||[]).reduce((s,f)=>s+f.paye,0)} F</div>
                        </div>
                        <div className="rounded p-2" style={{background:"#E6EAF2"}}>
                          <div className="text-[12px] font-medium">NOTES (enseignant → parent + directeur)</div>
                          <div className="text-[12px]">Notes: {e.notes.length} - Moy: {e.notes.length? (e.notes.reduce((s,n)=>s+n.note20,0)/e.notes.length).toFixed(1)+"/20" : "—"}</div>
                          <div className="text-[12px]">Messages: {e.messages.length}</div>
                        </div>
                      </div>
                    </div>
                  ))}
      ...
      {onglet === "matieres" && (
        <Panel title="Matières par Classe - Gestion complète (AMÉLIORÉ - Claude avait nombre seulement)" icon={FileText}>
          <p className="mb-2 text-[12px]" style={{color:"#9A8B67"}}>Claude: affichage nombre. NOUVEAU: édition complète par classe, 7 classes avec matières dont Kponjè.</p>
      ...
      {onglet === "frais" && (
        <Panel title="Frais Scolaires - Directeur fixe prix + MAJ auto élèves (AMÉLIORÉ)" icon={Wallet}>
          <p className="mb-3 text-[12px]" style={{color:"#9A8B67"}}>Claude: affichage seulement. NOUVEAU: éditable + quand directeur change prix, tous élèves mis à jour auto (payé/du). Secrétaire + parents voient instantanément.</p>
          <div className="space-y-2 mb-4">{fraisTypes.map((f,i)=>(
            <div key={i} className="flex gap-2 rounded-lg border px-3 py-2 items-center" style={{borderColor:"#E7DEC8"}}>
              <input defaultValue={f.libelle} onBlur={(e)=> mettreAJourFraisEtEleves(i,"libelle", e.target.value.trim())} className="flex-1 rounded border px-2 py-1 text-[14px]" />
              <input type="number" defaultValue={f.montant} onBlur={(e)=> mettreAJourFraisEtEleves(i,"montant", Number(e.target.value)||0)} className="w-24 rounded border px-2 py-1 text-right text-[14px]" />
              <Trash2 size={14} className="cursor-pointer" style={{color:"#8A2E2E"}} onClick={()=>{ if(confirm(`Supprimer ${f.libelle}?`)){ setFraisTypes(prev=>prev.filter((_,idx)=>idx!==i)); setEleves(prev=>prev.map(e=>({...e, frais: e.frais.filter(ff=>ff.libelle!==f.libelle)}))); } }} />
            </div>
          ))}</div>
          <div className="flex gap-2 rounded-lg p-3" style={{background:"#F1ECDD"}}>
            <input value={nouveauFrais.libelle} onChange={(e)=>setNouveauFrais({...nouveauFrais, libelle:e.target.value})} placeholder="Libellé ex: Cantine" className="flex-1 rounded border px-2 py-2 text-[14px]" />
            <input type="number" value={nouveauFrais.montant} onChange={(e)=>setNouveauFrais({...nouveauFrais, montant:e.target.value})} placeholder="Montant" className="w-28 rounded border px-2 py-2 text-[14px]" />
            <Btn variant="gold" small onClick={()=>{
              if(!nouveauFrais.libelle.trim()) return;
              const newF={ libelle:nouveauFrais.libelle.trim(), montant:Number(nouveauFrais.montant)||0 };
              setFraisTypes(prev=>[...prev, newF]);
              setEleves(prev=>prev.map(e=>({...e, frais: [...e.frais, {libelle:newF.libelle, du:newF.montant, paye:0}]})));
              setNouveauFrais({libelle:"", montant:""});
            }}>Ajouter + MAJ élèves</Btn>
          </div>
        </Panel>
      )}

      {onglet === "personnel" && (
        <Panel title="Personnel - Ajouter / Supprimer (Claude avait créé, nous avons amélioré avec téléphone)" icon={Users}>
          <p className="mb-2 text-[12px]" style={{color:"#9A8B67"}}>Claude avait liste basique. NOUVEAU: ajout avec téléphone + classe, et ce personnel sert à choisir enseignant dans Espace Enseignant.</p>
      ...
      {onglet === "archivage" && (
        <div className="space-y-5">
          <Panel title="Archivage Année Scolaire - Sauvegarde complète (NOUVEAU - nous avons ajouté)" icon={FileText}>
            <p className="mb-3 text-[12px]" style={{color:"#9A8B67"}}>NOUVEAU: Claude n'avait pas d'archivage. Nous avons ajouté archivage complet avec Supabase.</p>
            <div className="space-y-4">
              <div className="rounded-lg p-4" style={{background:"#FFFDF6", border:"1px solid #E7DEC8"}}>
                <div className="font-medium text-[15px]" style={{color:"#1B2A4A"}}>Année en cours: {anneeScolaire} - {eleves.length} élèves</div>
                <div className="mt-3 flex flex-wrap gap-2">
                  <Btn variant="gold" icon={FileText} onClick={archiverAnneeActuelle}>📦 Archiver {anneeScolaire}</Btn>
                  <Btn variant="primary" icon={Plus} onClick={demarrerNouvelleAnnee}>🆕 Démarrer nouvelle année</Btn>
                </div>
                {anneeArchiveeConf && <p className="mt-2 text-[13px]" style={{color:"#2F5233"}}>✓ Année archivée !</p>}
              </div>
            </div>
          </Panel>
          <Panel title="Années archivées" icon={Inbox}>
            <div className="space-y-3">
              {anneesArchivees.length===0 && <p style={{color:"#9A8B67"}}>Aucune année archivée.</p>}
              {anneesArchivees.map((arch,i)=>(
                <div key={i} className="rounded-xl border p-4" style={{borderColor:"#E7DEC8", background:"white"}}>
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="font-medium text-[15px]" style={{color:"#1B2A4A"}}>📚 {arch.annee} - {arch.effectif || (arch.eleves?.length || 0)} élèves</div>
                      <div className="text-[12px]" style={{color:"#9A8B67"}}>Archivée le {arch.dateArchivage}</div>
                    </div>
                    <div className="flex gap-2">
                      <Btn small variant="ghost" onClick={()=>restaurerArchive(arch)}>Restaurer</Btn>
                      <Trash2 size={14} className="cursor-pointer" style={{color:"#8A2E2E"}} onClick={()=>{ if(confirm(`Supprimer archive ${arch.annee} ?`)) setAnneesArchivees(prev=>prev.filter((_,idx)=>idx!==i)); }} />
                    </div>
                  </div>
                </div>
              ))}
      ...
      {onglet === "parametres" && (
        <div className="space-y-5">
          <Panel title="Informations École - Modifiable (Claude avait affichage seulement)" icon={School}>
            <p className="mb-2 text-[12px]" style={{color:"#9A8B67"}}>Claude: affichage. NOUVEAU: tout modifiable avec bouton Enregistrer.</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <Panel title="Codes d'accès - Directeur, Enseignant général, Secrétaire + Codes par CLASSE (NOUVEAU - chaque classe a son code)" icon={KeyRound}>
            <p className="mb-3 text-[12px]" style={{color:"#9A8B67"}}>NOUVEAU MAGNIFIQUE: Chaque classe a son propre code. Enseignant tape code de SA classe et arrive direct dans sa classe. Claude avait seulement 3 codes généraux.</p>
            ...
            <div className="mt-6">
              <div className="font-medium text-[15px] mb-2" style={{color:"#1B2A4A"}}>Codes par classe - Chaque enseignant tape le code de SA classe</div>
              ...
              <div className="mt-3 text-[12px]" style={{color:"#5C5240"}}>Exemple: Dis à enseignant CM1: "Tape code {codesClasses['CM1']||'1111'} pour aller direct en CM1"</div>
            </div>
